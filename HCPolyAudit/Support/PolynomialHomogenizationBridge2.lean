...
-/
import HCPolyAudit.Support.PolynomialHomogenizationBridge1
...
theorem isStationaryLaw_toRepoLaw {P : Measure (CoeffSpace d)}
    (h : IsStationaryLaw P) : HCPoly.Frozen.IsStationaryLaw (toRepoLaw P) := fun z =>
  (castMeasure_map (instMeasurableSpaceCoeffSpace_eq d) P (translateCoeff z)).trans
    ((castMeasure_inj (instMeasurableSpaceCoeffSpace_eq d) _ _).2 (h z))

theorem isUnitRangeLaw_toRepoLaw {P : Measure (CoeffSpace d)}
    (h : IsUnitRangeLaw P) : HCPoly.Frozen.IsUnitRangeLaw (toRepoLaw P) :=
  fun U V hU hV hUV =>
    coeffSigma_eq d U ▸ coeffSigma_eq d V ▸
      (indep_castMeasure (instMeasurableSpaceCoeffSpace_eq d) (coeffSigma d U) (coeffSigma d V)
          P).2 (h U V hU hV hUV)
...
theorem coarseEllipticityDagger_toRepoLaw {P : Measure (CoeffSpace d)} {g : ℝ}
  ...
  source_tail := fun t ht => (toRepoLaw_real P _).le.trans (h.source_tail t ht)
  coarse_bound := by
