...
/-!
# Theorem D: polynomial homogenization with a random source scale

Theorem D of *Homogenization at a polynomial scale in high contrast*
(`t.random.homogenization`), restated over Mathlib alone.

## The theorem in words

Fix a dimension `d ≥ 2`.  There are a tail exponent `c_d > 0` and a shape
constant `C₀` of the adapted domain, and then, for every coarse ellipticity
exponent `g ∈ [0, 1)`, constants `C > 0`, `c_src > 0`, `κ > 0` and `C₁`, such
that every law `P` satisfying the standing assumptions produces a homogenized
matrix `ā` with positive definite symmetric part `s̄`, a length
`L ∈ [1, (2 + Π K)^C]` polynomial in the reference aspect ratio
`e.reference.aspect.ratio` and in the growth witness of the gauge, a
homogenization scale `X ≥ 1` with the two-part tail
`P[X ≥ C L t] ≤ exp(-c_d t^{d - 2g}) + Ψ(c_src t)⁻¹` of `e.random.scale.tail`,
and a corrector family `(Φ_e, ∇Φ_e)` linear in the slope `e` and stationary
under the integer translations.  On one translation-invariant event of full
probability, and with the same scale and the same family throughout, the
following hold:

* the Dirichlet estimate `e.random.dirichlet`, on the adapted cells of `ā`;
* the corrector equation and the corrector estimate `e.random.corrector`;
* the Liouville classification `e.random.liouville.growth`, as a double
  inclusion;
* the large-scale estimates `e.random.energy` and `e.random.regularity`, on the
  ellipsoids `e.homogenized.ellipsoids`.
...
binder stands outside the exponent and outside the law, and the two radii enter
...
* **Symbols.**  The paper's `γ`, `Ψ_S`, `K_{Ψ_S}`, `ā`, `𝒳` and `φ_e` are here
  `g`, `Ψ`, `K`, `abar`, `X` and `Phi`, with gradients `gradPhi` and length
  `Lpoly`; only the spelling changes.
* **The homogenization scale.**  The paper asks `𝒳 ≥ max {1, S}`; here only
  `X ≥ 1` is asserted, which is weaker, the source scale still entering the
  tail.
...
* **The domains of the Dirichlet estimate.**  The paper states it for bounded
  Lipschitz `U ⊆ E_1`; here for the adapted cells `z + s̄^{1/2} □_j` normalized
  between two concentric adapted ellipsoids, which is every domain the argument
  uses.
...
/-- **Theorem D, `t.random.homogenization`: homogenization with a random source scale.** -/
theorem polynomial_homogenization
    -- the dimension
    (d : ℕ) (hd : 2 ≤ d) :
    -- the tail exponent `c(d)` and the shape constant `C₀`, chosen before `γ`
    ∃ (cd : ℝ) (C₀ : ℝ → ℝ → ℝ → ℝ), 0 < cd ∧
      (∀ s₀ ρ Rad : ℝ, 0 < C₀ s₀ ρ Rad) ∧
      -- the coarse ellipticity exponent `γ`
      ∀ g : ℝ, g ∈ Set.Ico (0 : ℝ) 1 →
        -- the constants `C`, `c_src`, `κ` and `C₁`, all chosen before the law
        ∃ (C cSrc κ : ℝ) (C₁ : ℝ → ℝ),
          0 < C ∧ 0 < cSrc ∧ 0 < κ ∧ (∀ ϑ : ℝ, 0 < C₁ ϑ) ∧
          -- the law, the reference block `𝐄`, the gauge `Ψ_S`, its growth
          -- witness `K_{Ψ_S}`, and the source scale `S`
          ∀ (P : MeasureTheory.Measure (CoeffSpace d)) (E : BlockMat d) (Ψ : ℝ → ℝ)
            ...
            IsStationaryLaw P →
            IsUnitRangeLaw P →
            -- `e.coarse.ellipticity` above the source scale, with `e.source.tail`
            CoarseEllipticityDagger P g E Ψ K S →
            -- `ā`, the length, the scale `𝒳` and the corrector family `φ_e`
            ∃ (abar : Mat d) (Lpoly : ℝ) (X : CoeffSpace d → ℝ)
              ...
              -- the length is polynomial: `L ≤ (2 + Π K)^C`
              Lpoly ≤ (2 + aspectRatio E * K) ^ C ∧
              -- the two-part tail `e.random.scale.tail`
              (∀ t : ℝ, 1 ≤ t →
              ...
              ∃ Ωend : Set (CoeffSpace d),
                ...
                -- (1) the Dirichlet estimate `e.random.dirichlet`, on the adapted
                -- cells of `ā`; the regularity exponent `s ∈ [(1 + γ)/4, 1/2)`
                ...
                -- (2b) the corrector estimate `e.random.corrector`,
                ...
                -- (4) the large-scale energy estimate `e.random.energy`,
