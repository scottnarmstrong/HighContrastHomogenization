...
Theorem B of *Homogenization at a polynomial scale in high contrast*
(`t.algebraic.convergence`), restated over Mathlib alone.  Every object the
statement names is rebuilt below, in the order in which the paper introduces it,
so the sections that follow read as the setup section of the paper and the last
one as the theorem.

## The theorem in words

Fix a dimension `d ≥ 2` and a coarse ellipticity exponent `g ∈ [0, 1)`.  There
are constants `C > 0` and `κ > 0` such that every law `P` satisfying the
standing assumptions admits a deterministic generation `m₀ ∈ ℕ` and a
deterministic doubled block `𝐀̄` with

* `m₀ ≤ ⌈C log₃ (2 + Π K)⌉`, the entry bound `e.algebraic.entry`, where `Π` is
  the reference aspect ratio `e.reference.aspect.ratio` and `K` is the growth
  witness of the source gauge;
* `Θ_{m₀+j} - 1 ≤ 3^{-κ j}` for every `j ∈ ℕ`, the contrast decay
  `e.algebraic.contrast.decay`, where `Θ_m` is the annealed contrast
  `e.Theta.m`;
* `𝐀̄` symmetric and positive definite, with
  `𝐀̄ ≤ 𝐀̄(□_{m₀+j}) ≤ (1 + 6 · 3^{-κ j}) 𝐀̄` for every `j ∈ ℕ`, the block decay
  `e.algebraic.block.decay`, where `𝐀̄(□_m)` is the annealed block of the
  centred cube;
* the Schur coefficients of `𝐀̄`, in the parametrization `e.annealed.schur`,
  satisfying `σ̄_* = σ̄` with `σ̄` positive definite and `κ̄` antisymmetric.

The constants `C` and `κ` depend on `d` and `g` alone: they are chosen before
the law, the reference block, the gauge, the growth witness and the source
scale, so the generation at which the algebraic rate sets in is reached at a
length polynomial in `2 + Π K` over the whole class.  No bound uniform as
`g ↑ 1` is asserted.
...
* **Symbols.**  The paper writes the exponent `γ`, the gauge `Ψ_S`, its growth
  witness `K_{Ψ_S}`, the limit block `𝐀̄` and its Schur coefficients `σ̄`, `κ̄`,
  `σ̄_*`.  Here they are `g`, `Ψ`, `K`, `Abar` and the Schur coefficients of
  `Abar`.  Only the spelling changes, the Lean names carrying no accents.
* **The convergence display.**  The paper writes
  `0 ≤ 𝐀̄(□_{m₀+j}) - 𝐀̄ ≤ 6 · 3^{-κ j} 𝐀̄`.  Here it is the equivalent pair of
  Loewner bounds `𝐀̄ ≤ 𝐀̄(□_{m₀+j})` and
  `𝐀̄(□_{m₀+j}) ≤ (1 + 6 · 3^{-κ j}) 𝐀̄`.  Subtracting doubled blocks is thereby
  avoided, and only the scalar dilation of a block is needed.
* **The limit block.**  The paper reads `𝐀̄` as a `2d × 2d` matrix.  Here it is
  carried in its four-block presentation, so its symmetry is the symmetry of the
  entries and its positive definiteness is the positivity of the doubled
  quadratic form.  Positive definiteness of the Schur coefficient `σ̄`, which is
  a `d × d` matrix, is Mathlib's own `Matrix.PosDef`.
* **The generation.**  The paper indexes the annealed blocks by generations
  `m₀ + j` in `ℕ`.  Here `m₀` and `j` are natural numbers and the scale they
  index is read as an integer.  The centred cube is defined at every integer
  scale, so the reading is faithful and nothing below the unit scale is lost.
...
namespace HCPoly.StatementAudit.AlgebraicConvergence
...
/-- **Theorem B, `t.algebraic.convergence`: algebraic convergence at a polynomial scale.** -/
theorem algebraic_convergence
    -- the dimension and the coarse ellipticity exponent `γ`
    (d : ℕ) (hd : 2 ≤ d) (g : ℝ) (hg : g ∈ Set.Ico (0 : ℝ) 1) :
    -- the constants `C = C(d, γ)` and `κ = κ(d, γ)`, chosen before the law and
    -- all of its data
    ∃ C κ : ℝ, 0 < C ∧ 0 < κ ∧
      ...
      ∀ (P : Measure (CoeffSpace d)) (E : BlockMat d) (Ψ : ℝ → ℝ) (K : ℝ)
        ...
        -- the deterministic generation `m₀` and the limit block `𝐀̄`
        ∃ (m₀ : ℕ) (Abar : BlockMat d),
          ...
          (∀ j : ℕ,
            annealedContrast P ((m₀ + j : ℕ) : ℤ) - 1 ≤
              (3 : ℝ) ^ (-κ * (j : ℝ))) ∧
          -- `𝐀̄` is symmetric and positive definite
          IsSymmetricBlockMat Abar ∧
          BlockPosDef Abar ∧
          -- `𝐀̄ ≤ 𝐀̄(□_{m₀+j}) ≤ (1 + 6 · 3^{-κ j}) 𝐀̄`,
          -- `e.algebraic.block.decay` as a pair of Loewner bounds
          (∀ j : ℕ,
          ...
          -- the Schur coefficients of `𝐀̄` in the parametrization
          -- `e.annealed.schur`: `σ̄_* = σ̄`, with `σ̄ > 0` and `κ̄` antisymmetric
...
end HCPoly.StatementAudit.AlgebraicConvergence
