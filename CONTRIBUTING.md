...
- **Never run `lake clean`.**  It wipes the `mathlib` and `CoarseGraining`
  ...
  `.lake/build/lib/lean/HCPoly` (and the corresponding
  `.lake/build/ir/HCPoly`) and re-run `lake build`.
...
- **The main results** are in `HCPoly/MainResults.lean`; the axiom audit is
...
- No `set_option maxHeartbeats`: a proof that exceeds the default budget is
  split or refactored.
- No bare `import Mathlib` inside `HCPoly`: modules import the precise
  Mathlib files they use.
- Comments cite the paper by its LaTeX labels only (for example
  `t.random.homogenization`), never by page or line.
