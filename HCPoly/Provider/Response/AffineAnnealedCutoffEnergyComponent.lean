...
-/
import HCPoly.Provider.Response.AffineResponseEnergyIdentification
import HCPoly.Provider.Response.AffineResponseOptimizer
import HCPoly.Provider.Response.AnnealedCutoffEnergyComponent

/-!
# The annealed cutoff-energy component on adapted cells

The affine reference family is chosen inside the proof.  Its child responses
cancel by physical stationarity, while its terminal response and difference
energy are identified with the physical adapted-cell quantities.  The final
statements therefore expose no reference-family or transport premise.
-/
...
private theorem cutoffWeightedTopHalfEnergy_affineResponse [NeZero d]
    ...
    (haRef : aRef.toCoeffField = affineCoefficient q
      ...
      (a.coeffOn (adaptedDomain hq t)).toCoeffField)
    (p r : Vec d) :
...
/-- The primal cutoff-energy contribution on the adapted terminal cell has the
three-term extended-nonnegative bound, with the physical response defect and
terminal response on the right. -/
theorem ofReal_abs_integral_affineSubSkewCutoffEnergyDefect_le_component
    ...
    (hH : H = t - s) (g : Mat d) (hg : IsSkewMat g)
    (p r : Vec d)
    ...
    ENNReal.ofReal |∫ a, (Book.Ch02.average (adaptedDomain hq t) (fun x ↦
      ...
      ENNReal.ofReal 2 * ENNReal.ofReal
          (Real.sqrt (profilePrimalResponseDefect P hq g hg s t p r)) *
        ...
        ENNReal.ofReal 4 * ENNReal.ofReal
          ...
          ENNReal.ofReal (Real.sqrt (∫ a, Book.Ch02.responseJ
            (adaptedDomain hq t) ((a.subSkew g hg).coeffOn
  ...
  obtain ⟨A, hA, hchild, hbase, hDint⟩ :=
    exists_integral_affineSubSkewDifferenceEnergy_eq_profilePrimal
          ...
          vecDot
            ...
            (matVecMul (((a.subSkew g hg).coeffOn
              ...
              ((centeredResponseOptimizer (adaptedDomain hq t)
                (a.subSkew g hg) p r).toH1.grad x)))) -
        Book.Ch02.responseJ (adaptedDomain hq t)
          ((a.subSkew g hg).coeffOn (adaptedDomain hq t)) p r := by
    intro a
    unfold X J Q pRef rRef
    rw [cutoffWeightedTopHalfEnergy_affineResponse hq t
  ...
  have hzero : ∫ a, W a ∂P = 0 := by
    exact integral_cutoffWeighted_affineSubSkewResponseJ_eq_zero hP hq hqGrid
      j hscale hls g hg A hA p r hchild hbase
        (integrableOn_adaptedPreYoungCutoff_pullback hq t Q)
        (cubeAverage_adaptedPreYoungCutoff_pullback_eq_one hq t)
  have hEJ := integral_affineSubSkewResponseJ_eq_physical
    (P := P) hq t g hg A hA p r
  have htau : ∫ a, T a ∂P =
      profilePrimalResponseDefect P hq g hg s t p r := by
    calc
      ∫ a, T a ∂P = ∫ a, D a ∂P := (integral_congr_ae hDT).symm
      _ = profilePrimalResponseDefect P hq g hg s t p r := hDint
  have hmain := ofReal_abs_integral_le_cutoffEnergyComponent_of_meanZero
