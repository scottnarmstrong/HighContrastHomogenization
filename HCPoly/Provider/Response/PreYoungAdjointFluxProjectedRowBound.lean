...
/-!
# Finite projected adjoint flux rows

The finite cube projections of the adjoint flux pairing are integrable, and
their annealed absolute values obey the transposed all-earlier row bound.
-/

/-- Finiteness of the adjoint weak quantity makes every absolute flux-slot
cell pairing integrable over the coefficient law. -/
theorem integrable_abs_adjoint_flux_cell_pairing_of_weak
...
/-- Every finite projected adjoint flux oscillation is almost-everywhere
strongly measurable under the coefficient law. -/
theorem aestrongly_measurable_adjoint_flux_projected_oscillation
...
/-- A finite projected adjoint flux oscillation is integrable, and its expected
absolute value is bounded by the corresponding finite weighted row. -/
theorem integrable_adjoint_flux_projected_oscillation_and_integral_abs_le_scale_rows
    [NeZero d] {P : Measure (CoeffSpace d)} [IsProbabilityMeasure P]
  ...
  have hintPair : ∀ j, ∀ z ∈ Z, ∀ w ∈ alignedIndex q (kOf j) s,
    ...
    simpa only [pair] using
      integrable_abs_adjoint_flux_cell_pairing_of_weak
  ...
  have hpath : ∀ a, |projected a| ≤ rhs a := by
    ...
    simpa only [projected, rhs, coeff, depth, pair, Z, kOf, hq] using
      abs_adjoint_flux_projected_oscillation_le_nested_depth_sum
        hq hst g hg p r Pcen N a
  have hprojectedMeas : AEStronglyMeasurable projected P := by
    simpa only [projected] using
      aestrongly_measurable_adjoint_flux_projected_oscillation
...
/-- The absolute lower integral of every finite adjoint flux projection has a
common bound by the transposed all-earlier hatted row. -/
theorem lintegral_adjoint_flux_projected_oscillation_le_row
    [NeZero d] {P : Measure (CoeffSpace d)} [IsProbabilityMeasure P]
    (hstat : HCPoly.Frozen.IsStationaryLaw P)
    {gamma : ℝ} {E : BlockMat d} {Psi : ℝ → ℝ} {K Cd : ℝ}
    {jStar M : ℤ} {Y : CoeffSpace d → ℝ}
  ...
  have hraw :=
    ofReal_abs_scaleSum_gradient_le_mul_profileAdjointHattedEarlierRow_rpow
