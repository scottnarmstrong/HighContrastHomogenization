...
/-!
# Finite projected adjoint rows, fixed grid

Fixed-rounded-grid sibling of `HCPoly.Provider.Response.PreYoungAdjointGradientProjectedRowBound`, `HCPoly.Provider.Response.PreYoungAdjointFluxProjectedRowBound`: the grid stays rounded at the fixed
  ...
  have hrowBound : ∀ j,
    ...
    simpa only [depth, pair, row, energy, load, Z, kOf, hq] using
      integral_double_avsum_le_of_half_integral_row_bounds
        ...
        (hintPair j) (fun w hw ↦
          (nestedAnnealedCellPairings_adjoint_le_of_cells
