...
/-!
# The physical primal projection limit

Finite projected primal pairings converge to the physical cutoff oscillation.
Their common annealed row bound passes to that limit by Fatou's lemma.
-/

/-- Finite-depth projected primal pairings converge pointwise to the physical pairing. -/
theorem tendsto_primal_projected_oscillation
...
/-- The physical primal pairing on one parent cell is its coordinate expansion. -/
theorem primal_physical_parent_pairing_eq_coordinate_sum
    [NeZero d] {q : Mat d} (hq : q.PosDef) {s t : ℤ} (hst : s ≤ t)
  ...
  have hcoordInt : ∀ i, IntegrableOn (fun x ↦ eta x * (X x).1 i) U volume := by
    intro i
    have hX := integrableOn_diagonalWeakState_fst_component_of_aligned
  ...
  simpa only [U, cut, X, eta] using
    primal_cutoff_oscillation_eq_unprojected_pairing
      hq s t z X Qcen

/-- A finite primal weak quantity makes the physical cutoff oscillation integrable. -/
theorem integrable_primal_physical_oscillation_of_weak
  ...
  let coord : (Fin d → ℤ) → Fin d → CoeffSpace d → ℝ := fun z i a ↦
    volumeAverage (adaptedCellAt q s z) (fun x ↦
      (adaptedPreYoungCutoff q hq t x -
        ...
        (diagonalWeakState hq t (a.subSkew g hg) p r x).1 i)
  have hread :=
