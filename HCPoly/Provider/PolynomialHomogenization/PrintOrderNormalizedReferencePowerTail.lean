...
/-!
# Power tails for an exactly normalized reference coefficient
...
exponent-gap factor.  This version retains the pointwise square-root estimate
and the halved decay rate derived inside the qualitative converter.
-/
...
private theorem exists_natural_rpow_absorption {kappa C : ℝ}
    (hkappa : 0 < kappa) (hC : 0 < C) :
    ∃ L : ℕ, C ≤ (3 : ℝ) ^ (kappa * (L : ℝ)) := by
  obtain ⟨L, hL⟩ := exists_nat_ge (Real.logb 3 C / kappa)
  refine ⟨L, ?_⟩
  have hlog : Real.logb 3 C ≤ kappa * (L : ℝ) := by
    rw [div_le_iff₀ hkappa] at hL
    simpa only [mul_comm] using hL
  have hmono :
      (3 : ℝ) ^ Real.logb 3 C ≤ (3 : ℝ) ^ (kappa * (L : ℝ)) :=
...
theorem exists_shiftedNormalizedReferencePowerTail_of_hasAllLaterPhysicalBlockRow
  ...
  obtain ⟨G, hG, _hGupper⟩ := Entry.exists_pow_three_bracket
  ...
  obtain ⟨L, hLabsorb⟩ := exists_natural_rpow_absorption hkappa hCpos
  refine ⟨aRef, L, haRefGlobal, ?_⟩
