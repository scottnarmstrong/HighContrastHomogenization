...
/-!
# From standard-cell bounds to adapted-cell bounds

A maximal standard-grid filling, countable response subadditivity, and the
Whitney row estimate transfer simultaneous standard-cell bounds to every
translate of a rounded adapted cell.
-/
...
/-- Simultaneous standard-cell response bounds imply the rounded adapted-cell
bound, with the explicit dimensional threshold supplied by the Whitney row. -/
theorem adapted_primal_of_standard
    ...
    BlockMatLoewnerLE
      ...
      (blockScale (boundaryConst Cd g n * Y * burnDiscount g jStar r) E) := by
  classical
  ...
  have hterm0 : ∀ i : I, 0 ≤ term i := by
    ...
    exact mul_nonneg (div_nonneg ENNReal.toReal_nonneg ENNReal.toReal_nonneg) hquad0
  ...
  have hrowWeight : ∀ u : ℕ,
    ...
    · subst u
      ...
      exact (Transport.sum_relative_volume_row_le_one hp Matrix.PosDef.one (hZ r)).trans
  ...
  have hrow : ∀ u : ℕ, (∑' w : {w // w ∈ Z (r - (u : ℤ))},
      ...
      (Cd * witnessEccentricity n * Y * eQuad) *
        ((3 : ℝ) ^ (-(u : ℤ)) * burnDiscount g jStar (r - (u : ℤ))) := by
    intro u
    rw [tsum_fintype]
    ...
    calc
      ∑ w ∈ Z (r - (u : ℤ)), rowTerm w ≤
          ∑ w ∈ Z (r - (u : ℤ)),
            ((volume (adaptedCellAt (1 : Mat d) (r - (u : ℤ)) w)).toReal /
                (volume (adaptedCellTranslate p r y)).toReal) *
              (Y * burnDiscount g jStar (r - (u : ℤ)) * eQuad) := by
        ...
        calc
          1 / 2 * blockVecDot X
              ...
              1 / 2 * blockVecDot X
                (blockMatVecMul (coarseBlock (standardCell d (r - (u : ℤ)) w) a) X) := by
            rw [show c ⟨u, ⟨w, hw⟩⟩ = standardCell d (r - (u : ℤ)) w from
              hidentity _ _]
          _ ≤ 1 / 2 * blockVecDot X
              (blockMatVecMul
                (blockScale (Y * burnDiscount g jStar (r - (u : ℤ))) E) X) :=
            hstandard _ _ (by
              rw [← hidentity]
              exact hsub _ _ hw) X
          _ = Y * burnDiscount g jStar (r - (u : ℤ)) * eQuad := by
      ...
      _ ≤ (Cd * witnessEccentricity n * (3 : ℝ) ^ (-(u : ℤ))) *
        ...
        exact mul_le_mul_of_nonneg_right (hrowWeight u)
          (mul_nonneg (mul_nonneg hY (by rw [burnDiscount]; positivity)) heQuad0)
      _ = (Cd * witnessEccentricity n * Y * eQuad) *
          ((3 : ℝ) ^ (-(u : ℤ)) * burnDiscount g jStar (r - (u : ℤ))) := by ring
  obtain ⟨hburnSummable, hburnTsum⟩ :=
    summable_zpow_mul_burnDiscount_and_tsum_le hg jStar r
  have hK0 : 0 ≤ Cd * witnessEccentricity n * Y * eQuad := by positivity
  have hrow0 : ∀ u : ℕ, 0 ≤ ∑' w : {w // w ∈ Z (r - (u : ℤ))}, term ⟨u, w⟩ :=
    fun u => tsum_nonneg fun w => hterm0 ⟨u, w⟩
  have hrowSummable : Summable fun u : ℕ =>
      ∑' w : {w // w ∈ Z (r - (u : ℤ))}, term ⟨u, w⟩ :=
    Summable.of_nonneg_of_le hrow0 hrow (hburnSummable.mul_left _)
  ...
  have hsubadd := blockQuadratic_le_tsum_weight_of_countable_aePartition
    (Transport.isOpenBoundedConvexDomain_adaptedCellTranslate hp r y) hparent0 a hc hcsub
    hcpair hcnull hc0 X htermSummable
  calc
    1 / 2 * blockVecDot X
    ...
    _ ≤ ∑' u : ℕ, (Cd * witnessEccentricity n * Y * eQuad) *
      ...
      hrowSummable.tsum_le_tsum hrow (hburnSummable.mul_left _)
    _ = (Cd * witnessEccentricity n * Y * eQuad) *
        (∑' u : ℕ, (3 : ℝ) ^ (-(u : ℤ)) *
          burnDiscount g jStar (r - (u : ℤ))) := by rw [tsum_mul_left]
    _ ≤ (Cd * witnessEccentricity n * Y * eQuad) *
        (zetaG g * burnDiscount g jStar r) :=
      mul_le_mul_of_nonneg_left hburnTsum hK0
    _ = 1 / 2 * blockVecDot X
        (blockMatVecMul
          (blockScale (boundaryConst Cd g n * Y * burnDiscount g jStar r) E) X) := by
      rw [Sharp.blockVecDot_blockMatVecMul_blockScale]
