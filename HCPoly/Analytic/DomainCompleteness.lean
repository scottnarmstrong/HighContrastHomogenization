...
domain admits one.  Together the two pin the class of domains the Dirichlet
estimate `e.random.dirichlet` covers
through its shape datum: it is exactly the class of nonempty bounded convex
domains, no wider and no narrower.
