...
/-!
# What the measurability condition of the local class excludes

`MemH1sLoc`, the local form of the coefficient Sobolev space of
`s.introduction`, asks of a pair `(v, Dv)` that it be measurable and that, on
every centred Euclidean ball, it be a weak-gradient pair approximated by smooth
functions in the `H¹_σ` norm.  This
module isolates the measurability conjunct and asks what it does.
