...
The coefficient-weighted spaces of `s.introduction` are
completions of `C^∞(U)`, the functions smooth *on* `U`, whereas the membership
classes used here ask for approximants smooth on all of `ℝ^d`.  The classical
bridge between the two dilates and then mollifies.  This module supplies the
first step and removes the need for the second.
