...
fields on `V` whose normalized norm is at most `1`.  A supremum over an index
family containing only the zero field is `0` for a reason that has nothing to do
with `F`, and an estimate on such a supremum would be
satisfied by the absence of anything to estimate.  This module shows that this
does not happen: over any domain containing a ball, each of the two families
contains a **nonzero** field.
