...
theorem synchronizedLogDetLoss_nonneg (d : ℕ) (hd : 2 ≤ d)
    ...
    0 ≤ synchronizedLogDetLoss P (Geometry.explicitRoundedGrid jStar m) h n := by
  unfold Homogenization.HighContrast.synchronizedLogDetLoss
...
theorem profile_add_determinantDrift_nonneg (d : ℕ) (hd : 2 ≤ d)
    ...
    0 ≤ profile P γ (Geometry.explicitRoundedGrid jStar m) jStar k n +
      determinantDrift P γ (Geometry.explicitRoundedGrid jStar m) jStar n := by exact add_nonneg (Annealed.bridge_profile_nonneg d hd P γ hγ E Ψ K S hstat hdag jStar hj m hm k n hk hkn) (Annealed.bridge_determinantDrift_nonneg d hd P γ E Ψ K S hstat hdag jStar hj m hm n)
...
theorem bridge_skolem (d : ℕ) (hd : 2 ≤ d) (γ : ℝ) (hγ : γ ∈ Set.Ico (0 : ℝ) 1) :
  ...
  obtain ⟨L₀, c₀, hc₀, Csrc, hCsrc, hbr⟩ :=
    Homogenization.HighContrast.Provider.successful_short_bridge d hd γ hγ
...
`Provider.fixed_geometry_one_grid_propagation_full` (`HCPoly/Entry/OneGridPropagation.lean`). -/
...
applied from `Provider.two_grid_transport`
                  ...
                  ∀ L : ℕ,
                    ...
                    profile P γ (Geometry.explicitRoundedGrid jStar mPlus) jStar (n + (L : ℤ))
                        ...
                        determinantDrift P γ (Geometry.explicitRoundedGrid jStar mPlus) jStar
                          (n + (L : ℤ)) ≤
                      C * (δ + ρ) :=
  Provider.two_grid_transport d hd γ hγ
