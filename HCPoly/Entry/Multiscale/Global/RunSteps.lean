...
`R3a` (Alternative 1, service), `R3e` (Alternative 3, synchronized) and `R3f`
(Alternative 3, long): the three clauses of `SelectionData.Selects` that keep the metric and
the retained generation `(𝔪, k)` and advance only `n`. All three are proved here; `run_step_long`
carries the extra binder `haQ` that the skeleton statement of the step omitted.
-/
...
`x > η` and the quarter propagation `x'@(n+h) ≤ x/4 + C Δ̂`; `h_step_decrease_quarter`
...
theorem run_step_service {d : ℕ} (hd : 2 ≤ d) (γ : ℝ) (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
    ...
    (hprop : profile P γ (Geometry.explicitRoundedGrid jStar st.m) jStar st.k (st.n + (S.h : ℤ)) +
      ...
      1 / 4 * (profile P γ (Geometry.explicitRoundedGrid jStar st.m) jStar st.k st.n +
        ...
        C * synchronizedLogDetLoss P (Geometry.explicitRoundedGrid jStar st.m) (S.h : ℤ) st.n)
  ...
  have hΔ0 : 0 ≤ synchronizedLogDetLoss P (Geometry.explicitRoundedGrid jStar st.m) (S.h : ℤ) st.n :=
    synchronizedLogDetLoss_nonneg d hd P γ E Ψ K Src hst hce jStar hjStar st.m st.hm
      (S.h : ℤ) st.n hSh_nonneg (by linarith [st.hk])
  ...
  have hx'0 : 0 ≤ profile P γ (Geometry.explicitRoundedGrid jStar st.m) jStar st.k
      (st.n + (S.h : ℤ)) +
      determinantDrift P γ (Geometry.explicitRoundedGrid jStar st.m) jStar (st.n + (S.h : ℤ)) :=
    profile_add_determinantDrift_nonneg d hd P γ hγ E Ψ K Src hst hce jStar hjStar st.m st.hm
      st.k (st.n + (S.h : ℤ)) st.hk (by linarith [st.hkn])
  have hstep_raw := h_step_decrease_quarter P γ jStar (S.eta ε σ) a c C st.m st.k st.n S.h
  ...
  have hgauge : runGauge P γ jStar (S.eta ε σ) a w S.h st.m st.k (st.n + (S.h : ℤ)) -
    ...
    exact run_energy_transfer
      ...
      (run_reserve (fun r => blockLogDet (adaptedMean P (Geometry.explicitRoundedGrid jStar st.m) r))
        S.h st.k (st.n + (S.h : ℤ)))
      c w (synchronizedLogDetLoss P (Geometry.explicitRoundedGrid jStar st.m) (S.h : ℤ) st.n) 0
      hwArg hstepArg hreserveArg
  have hkn' : st.k ≤ st.n + (S.h : ℤ) := by linarith [st.hkn]
...
`Provider.fixed_geometry_one_grid_propagation_full` instantiated at `h := S.h`, which needs
...
shape `1/8·e^{QΔ̂}·x + C(e^{QΔ̂} − 1)` is exactly `h_step_decrease.hprop`, so the target here
is `h_step_decrease` (`Containment.lean`), not the quarter variant. The charge
...
theorem run_step_sync {d : ℕ} (hd : 2 ≤ d) (γ : ℝ) (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
      ...
      n + (S.h : ℤ) ≤ m →
        profile P γ (Geometry.explicitRoundedGrid jStar metric) jStar n (m + (S.h : ℤ)) +
          ...
          1 / 8 * Real.exp ((bigQ d γ : ℝ) *
                synchronizedLogDetLoss P (Geometry.explicitRoundedGrid jStar metric) (S.h : ℤ) m) *
            ...
            Cone * (Real.exp ((bigQ d γ : ℝ) *
              synchronizedLogDetLoss P (Geometry.explicitRoundedGrid jStar metric) (S.h : ℤ) m) - 1))
    ...
    (hbig : profile P γ (Geometry.explicitRoundedGrid jStar st.m) jStar st.k st.n +
        determinantDrift P γ (Geometry.explicitRoundedGrid jStar st.m) jStar st.n > S.eta ε σ)
    (hobst : (d : ℝ)⁻¹ *
      synchronizedLogDetLoss P (Geometry.explicitRoundedGrid jStar st.m) (S.h : ℤ) st.n > σ)
    (hone' : st.k = st.n + (S.h : ℤ) →
      profile P γ (Geometry.explicitRoundedGrid jStar st.m) jStar (st.n + (S.h : ℤ))
          (st.n + (S.h : ℤ)) +
        determinantDrift P γ (Geometry.explicitRoundedGrid jStar st.m) jStar (st.n + (S.h : ℤ)) ≤ 1)
  ...
  have hexpnn : 0 ≤ Real.exp ((bigQ d γ : ℝ) *
      synchronizedLogDetLoss P (Geometry.explicitRoundedGrid jStar st.m) (S.h : ℤ) st.n) - 1 := by
  ...
  have hpropC : profile P γ (Geometry.explicitRoundedGrid jStar st.m) jStar st.k (st.n + (S.h : ℤ)) +
      determinantDrift P γ (Geometry.explicitRoundedGrid jStar st.m) jStar (st.n + (S.h : ℤ)) ≤
      1 / 8 * Real.exp ((bigQ d γ : ℝ) *
            synchronizedLogDetLoss P (Geometry.explicitRoundedGrid jStar st.m) (S.h : ℤ) st.n) *
          (profile P γ (Geometry.explicitRoundedGrid jStar st.m) jStar st.k st.n +
            determinantDrift P γ (Geometry.explicitRoundedGrid jStar st.m) jStar st.n) +
        C * (Real.exp ((bigQ d γ : ℝ) *
          synchronizedLogDetLoss P (Geometry.explicitRoundedGrid jStar st.m) (S.h : ℤ) st.n) - 1) := by
    have h2 : Cone * (Real.exp ((bigQ d γ : ℝ) *
        synchronizedLogDetLoss P (Geometry.explicitRoundedGrid jStar st.m) (S.h : ℤ) st.n) - 1) ≤
        C * (Real.exp ((bigQ d γ : ℝ) *
          synchronizedLogDetLoss P (Geometry.explicitRoundedGrid jStar st.m) (S.h : ℤ) st.n) - 1) :=
      mul_le_mul_of_nonneg_right hCone' hexpnn
    linarith [hprop1', h2]
  have hstep_raw := h_step_decrease P γ jStar (S.eta ε σ) a c C (bigQ d γ : ℝ) st.m st.k st.n
...
theorem run_step_long {d : ℕ} (hd : 2 ≤ d) (γ : ℝ) (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
    ...
    (hsmall : profile P γ (Geometry.explicitRoundedGrid jStar st.m) jStar st.k st.n +
        determinantDrift P γ (Geometry.explicitRoundedGrid jStar st.m) jStar st.n ≤ S.eta ε σ)
    (hobst : (d : ℝ)⁻¹ * logDetLoss P (Geometry.explicitRoundedGrid jStar st.m) st.n
  ...
  have hkn2 : st.k ≤ st.n + 2 * (S.L ε σ : ℤ) := by linarith [st.hkn, hL0]
  have hx'0 : 0 ≤ profile P γ (Geometry.explicitRoundedGrid jStar st.m) jStar st.k
      (st.n + 2 * (S.L ε σ : ℤ)) +
      determinantDrift P γ (Geometry.explicitRoundedGrid jStar st.m) jStar
        (st.n + 2 * (S.L ε σ : ℤ)) :=
  ...
  have hΔlt : (d : ℝ) * ε * σ <
      logDetLoss P (Geometry.explicitRoundedGrid jStar st.m) st.n (st.n + 2 * (S.L ε σ : ℤ)) :=
  ...
  have hadv := run_reserve_advance
    ...
    (jStar : ℤ) st.k st.n (st.n + 2 * (S.L ε σ : ℤ)) hmono hjn (by linarith [hkh, st.hk])
    (by linarith [hL0])
  have hreserveArg : logDetLoss P (Geometry.explicitRoundedGrid jStar st.m) st.n
      (st.n + 2 * (S.L ε σ : ℤ)) - 0 ≤
      run_reserve (fun r => blockLogDet (adaptedMean P (Geometry.explicitRoundedGrid jStar st.m) r))
          S.h st.k st.n -
        run_reserve (fun r => blockLogDet (adaptedMean P (Geometry.explicitRoundedGrid jStar st.m) r))
          S.h st.k (st.n + 2 * (S.L ε σ : ℤ)) := by
    have heq : logDetLoss P (Geometry.explicitRoundedGrid jStar st.m) st.n
        (st.n + 2 * (S.L ε σ : ℤ)) =
  ...
  have hgauge : runGauge P γ jStar (S.eta ε σ) a w S.h st.m st.k
    ...
    exact run_energy_transfer
      ...
      c w (logDetLoss P (Geometry.explicitRoundedGrid jStar st.m) st.n
        (st.n + 2 * (S.L ε σ : ℤ))) 0
      hwnn hstepArg hreserveArg
  have hpr' : projectiveDistance (1 : Mat d) st.m ≤ ε * (((st.i + 1 : ℕ) : ℝ) + 1) := by
    have h1 := st.hpr
