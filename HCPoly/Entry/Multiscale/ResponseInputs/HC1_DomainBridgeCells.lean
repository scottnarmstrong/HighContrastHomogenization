...
Section B1 only. See `HC1_DomainBridgeAnnealedBlock.lean` for the closing docstring and
`HC1_DomainBridge.lean` for the re-export.
-/
...
`HCPoly/Entry/Annealed/AdaptedDomainRecovery.lean` (`isOpenBoundedConvexDomain_affine_openCube`). -/
...
omit [NeZero d] in
/-- `U_t` is nonempty (it contains `0 = q 0`). -/
theorem adaptedCell_nonempty (q : Mat d) (j : ℤ) : (HighContrast.adaptedCell q j).Nonempty := by
  classical
...
def adaptedDomain (q : Mat d) (hq : IsUnit q) (j : ℤ) : Book.Ch02.Domain d :=
  { carrier := HighContrast.adaptedCell q j
    ...
    nonempty := adaptedCell_nonempty q j }
...
private theorem isEllipticMatrix_sub_skew {lam Lam : ℝ} {A : Mat d}
  ...
  have hkey : ∀ η : Vec d,
    ...
    have hcross : -(2 * vecDot (matVecMul A η) (matVecMul g η))
      ...
      rw [vecNormSq_add] at hnn; linarith
...
private theorem schurSkew_geoMean_swap_isSkew (X : FullBlockMat d) :
    matTranspose (schurSkew (ofFullBlockMat (GeoMean.geoMean X
        (toFullBlockMat (blockSwap d) * X⁻¹ * toFullBlockMat (blockSwap d)))))
      = -(schurSkew (ofFullBlockMat (GeoMean.geoMean X
  ...
  · by_cases hXp : X.PosSemidef
    · -- genuine case: `X` is positive definite, and the mean is swap self-dual
      ...
      have hBd : (R * X⁻¹ * R).PosDef := by
        ...
        rwa [hRH] at h
      set N := GeoMean.geoMean X (R * X⁻¹ * R) with hNdef
      have hNd : N.PosDef := GeoMean.geoMeanPosDef hXd hBd
      have hNu : IsUnit N.det := GeoMean.detUnit hNd
      have hric : N * X⁻¹ * N = R * X⁻¹ * R := GeoMean.geoMean_riccati hXd hBd
      ...
      have hric2 : (R * N⁻¹ * R) * X⁻¹ * (R * N⁻¹ * R) = R * X⁻¹ * R := by
        ...
        rw [hexp, hinv]
      have hdual : R * N⁻¹ * R = N := GeoMean.eq_geoMean_of_riccati hXd hNid hric2
      have h1 : N⁻¹ * R = R * N := by
    ...
    · -- junk branch: every `matSqrt` factor is `1`, so the mean is `1`
      ...
      have hgeo : GeoMean.geoMean X (R * X⁻¹ * R) = 1 := by
        rw [GeoMean.geoMean, hsX, hsXi]
        simp [hsB]
      rw [hgeo]
  ...
  · -- `X` is not invertible: `X⁻¹ = 0` and the mean collapses to `0`
    ...
    have hgeo : GeoMean.geoMean X (R * X⁻¹ * R) = 0 := by
      rw [GeoMean.geoMean, hXi, matSqrt_zero_eq]
...
`HCPoly/Entry/Annealed/AdaptedDomainRecovery.lean` supplies `lam Lam f` with `⇑a.1 =ᵐ f`.
