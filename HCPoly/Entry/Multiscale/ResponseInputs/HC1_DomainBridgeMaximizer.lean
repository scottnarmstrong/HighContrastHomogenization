import HCPoly.Entry.Multiscale.ResponseInputs.HC1_DomainBridgeCells
...
/-- Transport an `AHarmonicFunction` across an a.e. equality of coefficient fields. -/
def aHarmonicFunctionOfAEEqCoeff {U : Set (Vec d)} {a b : CoeffField d}
    (h : a =ᵐ[volumeMeasureOn U] b) (u : AHarmonicFunction a U) : AHarmonicFunction b U :=
  ⟨u.toH1, IsAHarmonicGradient.of_ae_eq_coeff h u.isHarmonic⟩
...
theorem nonempty_scalarCanonicalMaximizer_of_aeEq {U : Set (Vec d)} {a b : CoeffField d}
  ...
  obtain ⟨v⟩ := hv
  refine ⟨⟨⟨aHarmonicFunctionOfAEEqCoeff h v.toAHarmonicFunctionMeanZero.toAHarmonicFunction,
  ...
  have h1 : volumeAverage U (scalarResponseIntegrand U b p q w) =
      volumeAverage U (scalarResponseIntegrand U a p q (aHarmonicFunctionOfAEEqCoeff h.symm w)) :=
  ...
  have h2 : volumeAverage U
      ...
      volumeAverage U (scalarResponseIntegrand U b p q
        (aHarmonicFunctionOfAEEqCoeff h v.toAHarmonicFunctionMeanZero.toAHarmonicFunction)) :=
...
theorem nonempty_scalarCanonicalMaximizer_respCoeffMinus (q : Mat d) (hq : IsUnit q) (t : ℤ)
  ...
  have hbase : Nonempty (ScalarCanonicalMaximizer (HighContrast.adaptedCell q t) p r
    ...
    ScalarCanonicalMaximizer.nonempty_of_isOpenBoundedConvexDomain
      (adaptedCell_nonempty q t) (adaptedCell_isOpenBoundedConvexDomain q hq t) hEll' p r
...
`HC2_WeakSeminorm.lean`, which imports this file. The declarations here provide the copies
...
theorem standardCell_subset_centeredCube_of_mem_triadicIndexBox (t : ℤ) (n : ℕ)
  ...
  have hkey : ((m : ℝ) + 1 / 2) * c = 1 / 2 * (3 : ℝ) ^ t := by
    have hmhalf : (m : ℝ) + 1 / 2 = 1 / 2 * (3 : ℝ) ^ n := by linarith
...
/-- The defining set of `respWeakEnergy` (`AdaptedDefs.lean`), named so that its
...
theorem bddAbove_respWeakEnergySet (P : Measure (CoeffSpace d)) (qq : Mat d) (hq : IsUnit qq)
  ...
  set Udom : Book.Ch02.Domain d :=
    { carrier := U
      ...
      nonempty := adaptedCell_nonempty qq t } with hUdom
  have hgrad : ∀ (a : CoeffSpace d) (v w : AHarmonicFunction (b a) U),
    ...
    have hvT : IsResponseMaximizer U p q' f (aHarmonicFunctionOfAEEqCoeff hae v) := by
      intro z
      ...
      rw [h1, ← h2]
      exact key
    have hwT : IsResponseMaximizer U p q' f (aHarmonicFunctionOfAEEqCoeff hae w) := by
      intro z
      have key := hw (aHarmonicFunctionOfAEEqCoeff hae.symm z)
      have h1 : volumeAverage U (scalarResponseIntegrand U f p q' z) =
          volumeAverage U (scalarResponseIntegrand U (b a) p q'
            (aHarmonicFunctionOfAEEqCoeff hae.symm z)) :=
        volumeAverage_scalarResponseIntegrand_congr hae.symm p q' z _ (fun _ => rfl)
      have h2 : volumeAverage U (scalarResponseIntegrand U (b a) p q' w) =
          volumeAverage U (scalarResponseIntegrand U f p q'
            (aHarmonicFunctionOfAEEqCoeff hae w)) :=
    ...
    exact Book.Ch02.sameGradientAE_of_isResponseMaximizer (U := Udom) (a := A)
      (v := aHarmonicFunctionOfAEEqCoeff hae v) (w := aHarmonicFunctionOfAEEqCoeff hae w)
