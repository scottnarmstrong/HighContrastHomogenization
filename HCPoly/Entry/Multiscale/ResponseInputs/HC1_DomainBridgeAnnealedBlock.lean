...
omit [NeZero d] in
theorem isSymmetricBlockMat_blockCongr (G : BlockMat d) {A : BlockMat d}
  ...
  rw [Matrix.transpose_mul, Matrix.transpose_mul, Matrix.transpose_transpose,
    transpose_toFullBlockMat hA, Matrix.mul_assoc]
...
omit [NeZero d] in
/-- STEP 4.  `HasQuadraticMu` is unavoidable: `coarseBlockMatrix` is reconstructed from `Mu`
only through polarization. -/
theorem coarseBlockMatrix_sub_skew_eq_blockCongr {U : Set (Vec d)} {a : CoeffField d}
    {g : Mat d} (hg : matTranspose g = -g) (hquad : HasQuadraticMu U a) :
    coarseBlockMatrix U (fun x => a x - g)
      = blockCongr ⟨1, 0, g, 1⟩ (coarseBlockMatrix U a) :=
  (eq_coarseBlockMatrix_of_isCoarseBlockMatrix (isCoarseBlockMatrix_sub_skew hg hquad)).symm
...
omit [NeZero d] in
/-- AUX (helper, NEW).  `blockMatEntry` is the entry of the flattened block. -/
private theorem blockMatEntry_eq_toFull' (A : BlockMat d) (α β : BlockCoord d) :
    blockMatEntry A α β = toFullBlockMat A α β := by
  cases α <;> cases β <;> rfl
...
theorem integrable_blockMatEntry_coarseBlockMatrix_of_blockCongr
  ...
  have hint' : ∀ κ l : BlockCoord d,
    ...
    intro κ l
    simpa only [blockMatEntry_eq_toFull'] using hint κ l
  have key : ∀ a : CoeffSpace d, blockMatEntry (coarseBlockMatrix V (b a)) α β
    ...
    intro a
    rw [blockMatEntry_eq_toFull', hb a]
