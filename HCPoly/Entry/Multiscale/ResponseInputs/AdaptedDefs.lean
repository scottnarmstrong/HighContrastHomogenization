...
import HCPoly.Entry.Multiscale.ResponseInputs.Support
...
the substantive steps of the decomposition are proved in `AdaptedAlgebra` and its supporting
modules.
...
structure RawOutput (d : ℕ) (γ : ℝ) (S : SelectionData) (ε σ Cglob Cprof Csrc : ℝ) (H : ℕ)
  ...
  det : (d : ℝ)⁻¹ * logDetLoss P (Geometry.explicitRoundedGrid jStar (explicitCanonicalMetric F)) s t < σ
  prof : max
...
/-- `alpha = (1-gamma)/4` (`p.response.transfer`). -/
def respAlpha (γ : ℝ) : ℝ := (1 - γ) / 4

/-- `rho = (1+gamma)/2` (`p.response.transfer`). -/
def respRho (γ : ℝ) : ℝ := (1 + γ) / 2
...
def respM (A : BlockMat d) : Mat d :=
  GeoMean.geoMean (respBlockB A) (schurSigmaStar A)
...
def canonicalMean (A : BlockMat d) : BlockMat d :=
  ofFullBlockMat (GeoMean.geoMean (toFullBlockMat A)
...
replaced by the full operator norm `blockOpNorm` (`HCPoly/Entry/Setup/BlockCalculus.lean`).  It
...
(`HC3_CutoffKernel.lean`) constructs a member.
...
(`HC3_CutoffKernel.lean`) consumes smoothness and a second-derivative scale, so the class
