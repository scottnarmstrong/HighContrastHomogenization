...
`h6a_tailCell_of_bridge` supplies the per-cell hypothesis of the older-scale tail by carrying a
...
The port is `h6a_recentEnergyMap_port`, whose data are the parent optimizer restricted to each
...
The bound is the recent-difference port `h6a_recentEnergyMap_port` at a pointwise elliptic
...
theorem h6a_headCell_of_bridge (P : Measure (CoeffSpace d)) (γ : ℝ) (jStar : ℕ) (F : BlockMat d)
  ...
  have hport := h6a_recentEnergyMap_port (P := P) (γ := γ) (jStar := jStar) (F := F) (t := t)
