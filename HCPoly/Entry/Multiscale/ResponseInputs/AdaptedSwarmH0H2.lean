import HCPoly.Entry.Multiscale.ResponseInputs.AdaptedSwarmShear

/-!
# AdaptedSwarm, part 2 of 11
...
`logDetLoss P q s t = log det E_s - log det E_t` by definition.
...
theorem response_imbalance_comparison (d : ℕ) (_hd : 2 ≤ d) (γ : ℝ)
  ...
  have hts : toFullBlockMat (respMean P jStar F t) ≤ toFullBlockMat (respMean P jStar F s) :=
    full_le_of_block hEt.isHermitian hEs.isHermitian
  ...
  have hswapT : toFullBlockMat (blockSwap d) * (toFullBlockMat (respMean P jStar F t))⁻¹ *
    ...
    have h := full_le_of_block (Analysis.swapConj_posDef hEt).isHermitian hEt.isHermitian
  ...
  have hinv : (toFullBlockMat (respMean P jStar F t))⁻¹
    ...
    have h := matrix_inv_antitone hEs (posDef_smul hrpos hEt) hEsr
    rw [inv_smul_eq (ne_of_gt hrpos) ((Matrix.isUnit_iff_isUnit_det _).mp hEt.isUnit)] at h
  ...
  have hdiv : Real.log (respRatio P jStar F s t)
    ...
    simp only [respRatio]
    exact Real.log_div (ne_of_gt (by linarith)) (ne_of_gt (by linarith))
  have hloss : logDetLoss P (Geometry.explicitRoundedGrid jStar (explicitCanonicalMetric F)) s t
...
/-- Loewner order of the full representations transports back to `BlockMatLoewnerLE`. -/
theorem respCalib_blockLE_of_full {A B : BlockMat d}
    (h : toFullBlockMat A ≤ toFullBlockMat B) : BlockMatLoewnerLE A B := by
  intro X
  have hx := (Matrix.le_iff.mp h).dotProduct_mulVec_nonneg (toFullBlockVec X)
  simp only [Matrix.sub_mulVec, dotProduct_sub, star_trivial] at hx
  have hA : (toFullBlockVec X) ⬝ᵥ ((toFullBlockMat A).mulVec (toFullBlockVec X)) =
      blockVecDot X (blockMatVecMul A X) := by
    rw [← dotProduct_toFullBlockVec, toFullBlockVec_blockMatVecMul]
  have hB : (toFullBlockVec X) ⬝ᵥ ((toFullBlockMat B).mulVec (toFullBlockVec X)) =
...
private theorem respCalib_swapConj_mono {A B : FullBlockMat d} (h : A ≤ B) :
  ...
  have hR : (toFullBlockMat (blockSwap d))ᵀ = toFullBlockMat (blockSwap d) := by
    rw [← Matrix.conjTranspose_eq_transpose_of_trivial]
    exact (swap_hermitian d).eq
  have hc := congr_le h (toFullBlockMat (blockSwap d))
  rwa [hR] at hc
...
theorem respCalib_geoMean_le_left {ι : Type*} [Fintype ι] [DecidableEq ι]
    ...
    GeoMean.geoMean A B ≤ A := by
  have hm := GeoMean.geoMean_mono_right hA hB hA h
  rwa [GeoMean.geoMean_self hA] at hm
...
theorem respCalib_le_sqrt_smul_geoMean {ι : Type*} [Fintype ι] [DecidableEq ι]
    ...
    (h : A ≤ c • B) : A ≤ Real.sqrt c • GeoMean.geoMean A B := by
  have h1 : GeoMean.geoMean A A ≤ GeoMean.geoMean A (c • B) :=
    GeoMean.geoMean_mono_right hA hA (posDef_smul hc hB) h
  rw [GeoMean.geoMean_self hA] at h1
  have h2 : GeoMean.geoMean A (c • B) = Real.sqrt c • GeoMean.geoMean A B := by
    have hs := GeoMean.geoMean_smul hA hB (a := 1) (b := c) one_pos hc
...
theorem respCalib_canonicalMean_le {A F : FullBlockMat d}
    ...
    (h1 : A ≤ a • F) (h2 : F ≤ b • A) :
    GeoMean.geoMean A (toFullBlockMat (blockSwap d) * A⁻¹ * toFullBlockMat (blockSwap d)) ≤
      Real.sqrt (a * b) •
        GeoMean.geoMean F
  ...
  have hinv : A⁻¹ ≤ b • F⁻¹ := by
    have hstep := inv_antitone hF (posDef_smul hb hA) h2
    rw [inv_smul_eq hb.ne' (GeoMean.detUnit hA)] at hstep
  ...
  have hmono := GeoMean.geoMean_mono hA (posDef_smul ha hF) hRA
    (posDef_smul hb hRF) h1 hconj
  rwa [GeoMean.geoMean_smul hF hRF ha hb] at hmono
...
theorem respCalib_le_detRatio_smul {ι : Type*} [Fintype ι] [DecidableEq ι]
    ...
    B ≤ (B.det / A.det) • A := by
  have hP : (matSqrt A).PosDef := GeoMean.matSqrtPosDef' hA
  have hPP : matSqrt A * matSqrt A = A := (matSqrt_spec hA.posSemidef).2
  have hPh : (matSqrt A)ᴴ = matSqrt A := GeoMean.matSqrtHerm' hA.posSemidef
  have hPih : ((matSqrt A)⁻¹)ᴴ = (matSqrt A)⁻¹ := by
    rw [Matrix.conjTranspose_nonsing_inv, hPh]
  obtain ⟨hPR, hRP⟩ := GeoMean.sqrtCancel hA
  rw [GeoMean.matSqrtInv' hA] at hPR hRP
  have hX : ((matSqrt A)⁻¹ * B * (matSqrt A)⁻¹).PosDef := GeoMean.posDefConj' hB hP.inv
  have h1X : (1 : Matrix ι ι ℝ) ≤ (matSqrt A)⁻¹ * B * (matSqrt A)⁻¹ := by
    have hc := GeoMean.conjLe' hPih h
  ...
  have hc2 := GeoMean.conjLe' hPh hXle
...
theorem respCalib_canonicalMean_full (F : BlockMat d) :
    toFullBlockMat (canonicalMean F)
      = GeoMean.geoMean (toFullBlockMat F)
...
private theorem respCalib_explicitCanonicalMetric_eq (F : BlockMat d) :
  ...
  unfold explicitCanonicalMetric canonicalMean GeoMean.geoMean
  congr 2
...
private theorem respCalib_selfDual_mul {ι : Type*} [Fintype ι] [DecidableEq ι]
    ...
    (hric : M * A⁻¹ * M = R * A⁻¹ * R)
    (hMeq : GeoMean.geoMean A (R * A⁻¹ * R) = M) :
    M * R * M = R := by
  have hMM := Matrix.nonsing_inv_mul M (GeoMean.detUnit hM)
  have hMM' := Matrix.mul_nonsing_inv M (GeoMean.detUnit hM)
  ...
  have hself : R * M⁻¹ * R = M := by
    have hg := GeoMean.eq_geoMean_of_riccati hA hNpos hkey
    rwa [hMeq] at hg
...
private theorem respCalib_canonicalMean_selfDual {F : BlockMat d}
  ...
  have hM : (toFullBlockMat (canonicalMean F)).PosDef := by
    rw [respCalib_canonicalMean_full]; exact GeoMean.geoMeanPosDef hA hB
  have hric : toFullBlockMat (canonicalMean F) * (toFullBlockMat F)⁻¹ *
      toFullBlockMat (canonicalMean F) =
      toFullBlockMat (blockSwap d) * (toFullBlockMat F)⁻¹ * toFullBlockMat (blockSwap d) := by
    rw [respCalib_canonicalMean_full]; exact GeoMean.geoMean_riccati hA hB
...
private theorem respCalib_canonicalMean_posDef {F : BlockMat d} (hA : (toFullBlockMat F).PosDef) :
  ...
  exact GeoMean.geoMeanPosDef hA (respCalib_swapConj_posDef hA)
...
theorem respCalib_respg_isSkew {F : BlockMat d} (hA : (toFullBlockMat F).PosDef) :
  ...
  rw [full_eq (canonicalMean F), respCalib_swap_full, Matrix.fromBlocks_multiply,
...
private theorem respCalib_shear_congr {F : BlockMat d} (hA : (toFullBlockMat F).PosDef) :
  ...
  rw [respCalib_respG_full, full_eq (canonicalMean F), Matrix.fromBlocks_transpose,
