import HCPoly.Entry.Multiscale.ResponseInputs.HC2b_DiagonalWeakParentMapPortPlus
...
This module is the adjoint twin of `h6a_tailCell_of_bridge`: the parent optimizer is attached to
...
`h6a_parentEnergyMap_port_plus` proves the per-cell shape at a pointwise elliptic representative of
...
`aHarmonicFunctionOfAEEqCoeff` and the restriction `h6a_exists_restrict_solution_adaptedCellAtCenter`
produce the restricted Chapter-2 solution.  Finally `h6a_weakOptimizerEnergy_sq` and a.e. invariance
...
of `h6a_scaleTail_carrier`: the transported cell-average metric square of the parent optimizer field
is at most the printed geometric square times twice its energy average.  The bound is the adjoint
port `h6a_parentEnergyMap_port_plus` at a pointwise elliptic representative, transported back along
...
theorem h6a_tailCell_of_bridge_plus (P : Measure (CoeffSpace d)) (γ : ℝ) (jStar : ℕ)
  ...
  have hport := h6a_parentEnergyMap_port_plus (P := P) (γ := γ) (jStar := jStar) (F := F) (t := t)
