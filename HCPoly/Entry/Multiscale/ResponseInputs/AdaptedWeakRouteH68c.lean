import HCPoly.Entry.Multiscale.ResponseInputs.AdaptedWeakRouteH68b
...
* `D_{n,w} = (N_{n,w} - I) - (N_0 - I)` with `N = normalizedBlock(·, E_t)`, using
  `normalizedBlock_blockSub` and `b130_adaptedCellAtCenter_zero`;
...
* the recentred field `a_∓` and the normalizer `Ehat^∓ = blockCongr (respG F) E_t` are the
  SAME congruence of the `E_t`-picture (`h68_coarseBlockMatrix_respCoeffMinus_at` /
...
Conjunct 1 (measurability) is `h68_aestronglyMeasurable_recentDefectBlock_minus/plus` above,
carrier-independent.  Conjunct 2 is the same pointwise bound.

All three conjuncts, both signs.  `K n :=
(3 ^ (respRho γ * n) + 1) ^ 2 * Cm ^ (2/Q)`, positive.  The chain above:

* `h68_blockSpecBound_le_blockOpNorm` -- `blockSpecBound N ≤ blockOpNorm N` UNCONDITIONALLY, so
  the new hypothesis implies the old one and no `respAllScaleMax` consumer is lost;
* `h68_blockOpNorm_sub_id_le_specBound_add_two` and `b179_pathwise_envelope_abs` -- the
  two-sided defining set is a.e. `BddAbove`, the `respAllScaleAbs` analogue;
* `h68_weighted_blockOpNorm_le_respAllScaleAbs` -- the `le_csSup` step;
* `h68_blockOpNorm_normalizedBlock_sub_le` -- the split `(N_{n,w} - I) - (N_0 - I)` in the
  `E_t`-picture, which is where BOTH sides are needed and where the one-sided carrier fails;
* `h68_blockOpNorm_normalizedBlock_blockCongr_le` -- congruence invariance:
...
* `h68_blockCongr_posDef` (with `b179_isUnit_respG` / `b179_isUnit_respGPlus`) -- positive
  definiteness of `Ehat^∓`, discharged, not assumed;
* `h68_recentDefect_le_respAllScaleAbs_minus` / `_plus` -- THE η-KERNEL POINTWISE BOUND,
  `recentDefect ≤ (3 ^ (ρ n) + 1) · respAllScaleAbs a` a.e., uniformly in `n` and `w`;
* `h68_respAllScaleAbs_measurable`, `h68_respAllScaleAbs_nonneg`,
  `h68_integral_sq_le_rpow` (Jensen, `∫ f ^ 2 ≤ (∫ f ^ Q) ^ (2/Q)` on a probability measure) and
  `b179_conjuncts_of_pointwise` -- integrability (conjunct 2) and the exponent (conjunct 3).

Conjunct 1 is `h68_aestronglyMeasurable_recentDefectBlock_minus` / `_plus`, carrier-independent.
...
theorem integral_recentDefect_sq_le_minus (d : ℕ) (hd : 2 ≤ d) (γ : ℝ)
  ...
  refine ⟨fun n => ((3 : ℝ) ^ (respRho γ * (n : ℝ)) + 1) ^ 2 *
      Cm ^ ((2 : ℝ) / (bigQ d γ : ℝ)), fun n => ?_, ?_⟩
  · show (0 : ℝ) < ((3 : ℝ) ^ (respRho γ * (n : ℝ)) + 1) ^ 2 * Cm ^ ((2 : ℝ) / (bigQ d γ : ℝ))
    have h1 : (0 : ℝ) < (3 : ℝ) ^ (respRho γ * (n : ℝ)) := Real.rpow_pos_of_pos (by norm_num) _
  ...
  · intro ε σ Cglob Cprof Csrc Bresp H P E Ψ Kg Src B jStar F s t raw _hcal η hη hAint hAle n w hw
    ...
    have hmeas1 := h68_aestronglyMeasurable_recentDefectBlock_minus hq P t n w (respEhatMinus P jStar F t) F
    ...
    have hbd := h68_recentDefect_le_respAllScaleAbs_minus hd γ hγ P E Ψ Kg Src
    ...
    have hc : (0 : ℝ) ≤ (3 : ℝ) ^ (respRho γ * (n : ℝ)) + 1 := by
      have := Real.rpow_nonneg (by norm_num : (0 : ℝ) ≤ 3) (respRho γ * (n : ℝ))
      linarith only [this]
    obtain ⟨hint, hle⟩ := b179_conjuncts_of_pointwise P (bigQ d γ) hQ2 Cm η hCm hη.1 _ _
      (h68_respAllScaleAbs_nonneg P γ jStar F t) hg0 hgmeas hAmeas _ hc hbound hAint hAle
...
theorem integral_recentDefect_sq_le_plus (d : ℕ) (hd : 2 ≤ d) (γ : ℝ)
  ...
  · intro ε σ Cglob Cprof Csrc Bresp H P E Ψ Kg Src B jStar F s t raw _hcal η hη hAint hAle n w hw
    ...
    have hq : IsUnit (respGrid jStar F) := Geometry.isUnit_roundedGrid raw.hj hm
    have hQ2 : 2 ≤ bigQ d γ := bigQ_two_le d hd γ hγ
    have hmeas1 := h68_aestronglyMeasurable_recentDefectBlock_plus hq P t n w (respEhatPlus P jStar F t) F
    have hg0 : ∀ a : CoeffSpace d, 0 ≤ recentDefect (respGrid jStar F) t n w
    ...
    have hAmeas : AEStronglyMeasurable (fun a : CoeffSpace d =>
        respAllScaleAbs P γ jStar F t a ^ 2) P :=
      ((h68_respAllScaleAbs_measurable P γ jStar F hq t).pow_const 2).aestronglyMeasurable
    have hbd := h68_recentDefect_le_respAllScaleAbs_plus hd γ hγ P E Ψ Kg Src
      raw.stat raw.ell jStar raw.hj F hm t
    have hbound : ∀ᵐ a ∂P, recentDefect (respGrid jStar F) t n w
        (respEhatPlus P jStar F t) (respCoeffPlus F a)
        ≤ ((3 : ℝ) ^ (respRho γ * (n : ℝ)) + 1) * respAllScaleAbs P γ jStar F t a := by
...
(`AdaptedSwarm.lean`).  The two corollaries below discharge both premises and leave
...
`response_allscale_abs` and `response_source_smallness` (`AdaptedAssembly.lean`).  They show
...
reconciled by the tree's `max` idiom, as `AdaptedAssembly.lean` does. -/

/-- `RawOutput` is antitone in the source threshold constant `Csrc`.  Same statement as the
`private` `rawOutput_le_csrc` (`AdaptedAssembly.lean`), which is downstream of this
file. -/
private theorem t24_rawOutput_le_csrc {γ : ℝ} {S : SelectionData}
...
theorem integral_recentDefect_sq_le_of_rawOutput_minus (d : ℕ) (hd : 2 ≤ d) (γ : ℝ)
  ...
  have hrawA := t24_rawOutput_le_csrc raw (le_max_left CsrcA CsrcI)
  have hrawI := t24_rawOutput_le_csrc raw (le_max_right CsrcA CsrcI)
...
private theorem h69_integral_le_of_sq_le {α : Type*} [MeasurableSpace α] (P : Measure α)
...
theorem integral_weakSums_sq_le (rho : ℝ) (hrho : 0 ≤ rho) (K : ℕ → ℝ) (hK : ∀ n, 0 < K n)
  ...
  have hav_bd : ∀ n, n ≤ H →
    ...
    have hDbd : ∀ w ∈ triadicIndexBox d n,
      ...
      intro w hw
      refine h69_integral_le_of_sq_le P (fun a => DD n w a) (hDint n hn w hw)
