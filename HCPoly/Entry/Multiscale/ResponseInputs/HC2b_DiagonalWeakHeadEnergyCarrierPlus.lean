...
theorem h6a_headEnergy_carrier_plus (P : Measure (CoeffSpace d)) (jStar : ℕ) (F : BlockMat d)
  ...
  have hRespV : ∀ w ∈ Z, ResponseJ (adaptedCellAtCenter q (t - (n : ℤ)) w) p r b
    ...
    intro w _
    exact h6a_responseJ_adaptedCellAtCenter_respCoeffPlus q hgrid (t - (n : ℤ)) w F a p r
  have hRespU : ResponseJ (HighContrast.adaptedCell q t) p r b
      = (1 / 2 : ℝ) * QU - vecDot p r := by
    exact h6a_responseJ_adaptedCell_respCoeffPlus q hgrid t F a p r
  ...
  have hLsq : respLsqPlus P jStar F t e = blockVecDot x (blockMatVecMul E x) := by
    rw [h6a_respLsqPlus_eq_quadratic]
    rfl
