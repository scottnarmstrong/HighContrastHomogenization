...
omit [NeZero d] in
/-- Congruence in the quadratic form. -/
theorem blockVecDot_blockCongr (G A : BlockMat d) (P : BlockVec d) :
    blockVecDot P (blockMatVecMul (blockCongr G A) P) =
      blockVecDot (blockMatVecMul G P) (blockMatVecMul A (blockMatVecMul G P)) := by
  have hL : blockVecDot P (blockMatVecMul (blockCongr G A) P)
