...
the analytic kernels are in `HCPoly/Entry/Multiscale/ResponseInputs/Kernels.lean`.
...
theorem loewner_swapConj_of_canonicalImbalance_le {d : ℕ} (_hd : 2 ≤ d) (A : BlockMat d)
  ...
  have hfull : toFullBlockMat A ≤ toFullBlockMat B := by
    simpa only [B, full_blockScale, toFullBlockMat_ofFullBlockMat] using
      le_of_imbalance_le (full_posDef hAs hA) hc
...
`0 < X ≤ Y`, `𝐑Y⁻¹𝐑 ≤ 𝐑X⁻¹𝐑`.  `inv_antitone` (helpers) gives `Y⁻¹ ≤ X⁻¹`; congruence by the
...
theorem swapConj_inv_antitone {d : ℕ} (X Y : BlockMat d)
  ...
  have hinv : (toFullBlockMat Y)⁻¹ ≤ (toFullBlockMat X)⁻¹ :=
    matrix_inv_antitone hXf hYf hXYf
  have hRt : Rᵀ = R := by
    ext (i | i) (j | j) <;>
  ...
  have hconj : R * (toFullBlockMat Y)⁻¹ * R ≤
    ...
    simpa [R, hRt] using congr_le hinv R
  intro v
...
theorem adaptedMean_persistence (d : ℕ) (hd : 2 ≤ d)
  ...
  have hAtSym : IsSymmetricBlockMat At := by
    simpa [At, adaptedMean] using
      Annealed.isSymmetricBlockMat_annealedBlock P
