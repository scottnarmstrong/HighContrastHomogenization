import HCPoly.Entry.Multiscale.ResponseInputs.AdaptedSwarmH0H2

/-!
# AdaptedSwarm, part 3 of 11
...
Route: the Riccati identity `GeoMean.geoMean_riccati` gives `M(E) <= E <= d(E)^{1/2} M(E)`
(`p.response.transfer`); then `raw.calib_lo`/`raw.calib_hi` at `s`, the comparison `E_t <= E_s <= r E_t`
of `response_imbalance_comparison`, and `GeoMean.geoMean_mono`.  The congruence cost is
...
theorem response_calibrated_blocks (d : ℕ) (_hd : 2 ≤ d) (γ : ℝ)
  ...
  have hcsC : Real.sqrt ((1 + Real.sqrt ε * σ) * (1 - Real.sqrt ε * σ)⁻¹)
    ...
    calc (1 + Real.sqrt ε * σ) * (1 - Real.sqrt ε * σ)⁻¹
      ...
      _ = (1 + Real.sqrt S.eps0 * S.eps0) * Real.exp (d : ℝ) *
            (1 - Real.sqrt S.eps0 * S.eps0)⁻¹ := by ring
  -- matrix data
  have hAf : (toFullBlockMat F).PosDef := full_posDef raw.symm raw.pos
  ...
  have hsharp : ∀ u : ℤ, toFullBlockMat (blockSwap d) *
    ...
    have hle := full_le_of_block hh hpos.isHermitian hswap
  ...
  have hcal_hi : toFullBlockMat (respMean P jStar F s) ≤
    ...
    have h := full_le_of_block hAs.isHermitian
      ((Analysis.toFullBlockMat_isHermitian_iff _).2 (isSymm_blockScale _ raw.symm)) raw.calib_hi
    rwa [full_blockScale] at h
  have hcal_lo : (1 - Real.sqrt ε * σ) • toFullBlockMat F ≤
      toFullBlockMat (respMean P jStar F s) := by
    have h := full_le_of_block
      ((Analysis.toFullBlockMat_isHermitian_iff _).2 (isSymm_blockScale _ raw.symm))
  ...
  have hts : toFullBlockMat (respMean P jStar F t) ≤ toFullBlockMat (respMean P jStar F s) :=
    full_le_of_block hAt.isHermitian hAs.isHermitian
  ...
  have hGM : (toFullBlockMat (respG F))ᵀ *
      GeoMean.geoMean (toFullBlockMat F) (toFullBlockMat (blockSwap d) *
  ...
  have hM0psd : (toFullBlockMat (respM0 F)).PosSemidef := by
    rw [← hGM]
    have hx := (GeoMean.geoMeanPosDef hAf hBf).posSemidef.conjTranspose_mul_mul_same
      (toFullBlockMat (respG F))
    simpa only [Matrix.conjTranspose_eq_transpose_of_trivial] using hx
  have hlow := respCalib_lower (GeoMean.geoMeanPosDef hAt hBt).posSemidef hGM hCpos hbaC hNf hNtA
  ...
  have hups := respCalib_upper hM0psd hGM (Real.sqrt_nonneg (respKappa P jStar F s)) hcsC
    hAsN hNs
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_⟩
  · refine respCalib_blockLE_of_full ?_
    rw [full_blockScale, respCalib_ehatMinus_full]
    exact hlow
  · refine respCalib_blockLE_of_full ?_
    rw [full_blockScale, respCalib_ehatPlus_full, respCalib_ehatMinus_full]
    have hD := congr_le hlow (toFullBlockMat (blockD d))
  ...
  · refine respCalib_blockLE_of_full ?_
    rw [full_blockScale, respCalib_ehatPlus_full, respCalib_ehatMinus_full]
    have hD := congr_le hupt (toFullBlockMat (blockD d))
    rwa [Matrix.mul_smul, Matrix.smul_mul, respCalib_blockD_congr_respM0] at hD
  · refine respCalib_blockLE_of_full ?_
    rw [full_blockScale, respCalib_ehatMinus_full]
    exact hups
  · refine respCalib_blockLE_of_full ?_
    rw [full_blockScale, respCalib_ehatPlus_full, respCalib_ehatMinus_full]
    have hD := congr_le hups (toFullBlockMat (blockD d))
    rwa [Matrix.mul_smul, Matrix.smul_mul, respCalib_blockD_congr_respM0] at hD
...
/-- Congruence acts on the quadratic form by transporting the vector. -/
theorem h3_blockVecDot_blockCongr (G A : BlockMat d) (X : BlockVec d) :
    blockVecDot X (blockMatVecMul (blockCongr G A) X)
      = blockVecDot (blockMatVecMul G X) (blockMatVecMul A (blockMatVecMul G X)) := by
  simp only [blockCongr, ← dotProduct_toFullBlockVec, toFullBlockVec_blockMatVecMul,
    toFullBlockMat_ofFullBlockMat, Matrix.mulVec_mulVec]
  rw [Matrix.mul_assoc, Matrix.dotProduct_mulVec, ← Matrix.vecMul_vecMul,
    Matrix.vecMul_transpose, ← Matrix.dotProduct_mulVec]

/-- `G = ((Id,0),(g,Id))` acts as `(x1, x2) => (x1, g x1 + x2)`. -/
private theorem h3_blockMatVecMul_respG (F : BlockMat d) (X : BlockVec d) :
...
private theorem h3_vecDot_respP_respQ {A : BlockMat d} (hM : (respM A).PosDef) (e : Vec d) :
...
/-- `respSkew` is skew. -/
private theorem h3_respSkew_isSkew (A : BlockMat d) :
...
private theorem h3_respG_respxMinus (P : Measure (CoeffSpace d)) (jStar : ℕ) (F : BlockMat d)
    (t : ℤ) (e : Vec d) :
  ...
  rw [h3_blockMatVecMul_respG]
  simp only [respxMinus, respqMinus, h3_matVecMul_sub, h3_matVecMul_neg, Prod.mk.injEq]
  refine ⟨trivial, ?_⟩
  funext i
...
theorem h3_respLsqMinus_eq (P : Measure (CoeffSpace d)) (jStar : ℕ) (F : BlockMat d)
    (t : ℤ) (e : Vec d) :
  ...
  rw [respLsqMinus, respEhatMinus, h3_blockVecDot_blockCongr, h3_respG_respxMinus]

/-- `D = diag(Id,-Id)` acts as `(x1,x2) => (x1,-x2)`. -/
private theorem h3_blockMatVecMul_blockD (X : BlockVec d) :
...
/-- `(L^+)^2 = (-p, -(q + h_t p)). E_t (-p, -(q + h_t p))`. -/
theorem h3_respLsqPlus_eq (P : Measure (CoeffSpace d)) (jStar : ℕ) (F : BlockMat d)
    (t : ℤ) (e : Vec d) :
  ...
  have hfst : (blockMatVecMul (respG F)
      ...
      = -(respP (respMean P jStar F t) e) := by
    rw [h3_blockMatVecMul_blockD, h3_blockMatVecMul_respG]
    rfl
  have hsnd : (blockMatVecMul (respG F)
      ...
      = -(respQ (respMean P jStar F t) e
          + matVecMul (respSkew (respMean P jStar F t)) (respP (respMean P jStar F t) e)) := by
    rw [h3_blockMatVecMul_blockD, h3_blockMatVecMul_respG]
    simp only [respxPlus, respqPlus, h3_matVecMul_sub, h3_matVecMul_neg]
    funext i
    simp only [Pi.add_apply, Pi.sub_apply, Pi.neg_apply]
    ring
  ...
  rw [respLsqPlus, respEhatPlus, blockAdjoint, h3_blockVecDot_blockCongr, respEhatMinus,
    h3_blockVecDot_blockCongr, hv]

/-- Entries of `annealedBlockOf` are the expectations of the pathwise entries. -/
private theorem h3_blockMatEntry_annealedBlockOf (P : Measure (CoeffSpace d))
...
private theorem h3_blockVecDot_entries (A : BlockMat d) (X : BlockVec d) :
...
private theorem h3_integral_blockVecDot (P : Measure (CoeffSpace d))
  ...
  simp only [h3_blockVecDot_entries, h3_blockMatEntry_annealedBlockOf]
...
private theorem h3_vecDot_add (x y z : Vec d) :
...
theorem h3_vecDot_respP_respqMinus (P : Measure (CoeffSpace d)) (jStar : ℕ)
  ...
  have h2 := h3_respSkew_isSkew (respMean P jStar F t)
  have hskew : matTranspose (respg F - respSkew (respMean P jStar F t))
    ...
    rw [hg, h2]; abel
  rw [respqMinus, h3_vecDot_add, h3_vecDot_respP_respQ hM, he,
    h3_vecDot_skew_self _ hskew, add_zero]

/-- The reciprocal normalizations: `p. q^+ = 1`. -/
theorem h3_vecDot_respP_respqPlus (P : Measure (CoeffSpace d)) (jStar : ℕ)
    (F : BlockMat d) (t : ℤ) (e : Vec d) (he : vecDot e e = 1)
    (hM : (respM (respMean P jStar F t)).PosDef)
    (hg : matTranspose (respg F) = -(respg F)) :
...
(`p.response.transfer`). -/
theorem h3_respEJMinus_eq (P : Measure (CoeffSpace d)) [IsProbabilityMeasure P]
  ...
  have hpq := h3_vecDot_respP_respqMinus P jStar F t e he hM hg
  have hQint := h3_integrable_blockVecDot P (respCell jStar F t) (respCoeffMinus F)
    (respxMinus P jStar F t e) hint
  have hQ := h3_integral_blockVecDot P (respCell jStar F t) (respCoeffMinus F)
    (respxMinus P jStar F t e) hint
...
theorem h3_respEJPlus_eq (P : Measure (CoeffSpace d)) [IsProbabilityMeasure P]
    (jStar : ℕ) (F : BlockMat d) (t : ℤ) (e : Vec d) (he : vecDot e e = 1)
    (hM : (respM (respMean P jStar F t)).PosDef)
    (hg : matTranspose (respg F) = -(respg F))
    (hpath : ∀ a : CoeffSpace d,
  ...
  have hpq := h3_vecDot_respP_respqPlus P jStar F t e he hM hg
  have hQint := h3_integrable_blockVecDot P (respCell jStar F t) (respCoeffPlus F)
    (respxPlus P jStar F t e) hint
  have hQ := h3_integral_blockVecDot P (respCell jStar F t) (respCoeffPlus F)
    (respxPlus P jStar F t e) hint
  have huniv : P.real Set.univ = 1 := by simp
  rw [respEJPlus]
  simp only [hpath, hpq]
...
theorem h3_integral_respJ (P : Measure (CoeffSpace d)) [IsProbabilityMeasure P]
  ...
  have huniv : P.real Set.univ = 1 := by simp
  have hQint := h3_integrable_blockVecDot P (HighContrast.adaptedCell qq u) b X hint
  have hQ := h3_integral_blockVecDot P (HighContrast.adaptedCell qq u) b X hint
  simp only [hpath]
  rw [integral_sub (hQint.const_mul _) (integrable_const _), integral_const_mul, hQ, hann,
    integral_const, huniv]
  simp
...
theorem h3_respTauMinus_eq (P : Measure (CoeffSpace d)) [IsProbabilityMeasure P]
...
theorem h3_zero_le_respTauMinus (P : Measure (CoeffSpace d)) (jStar : ℕ)
...
theorem h3_respEhatMinus_mono (P : Measure (CoeffSpace d)) (jStar : ℕ) (F : BlockMat d)
