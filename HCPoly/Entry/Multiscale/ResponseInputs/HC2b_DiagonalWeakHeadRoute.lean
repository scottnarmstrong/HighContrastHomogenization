...
import HCPoly.Entry.Multiscale.ResponseInputs.HC2b_DiagonalWeakHeadCell
import HCPoly.Entry.Multiscale.ResponseInputs.HC2b_DiagonalWeakHeadEnergyCarrier
...
load. -/
theorem h6a_recentHead_carrier_minus (P : Measure (CoeffSpace d)) [IsProbabilityMeasure P]
  ...
  have h1u : ∀ n ∈ Finset.range (H + 1), ∀ w ∈ triadicIndexBox d n, ∀ j : Fin d,
    ...
    (h6a_integrableOn_optimizerField_respCoeffMinus_box (respGrid jStar F) hgrid t F a u n w
      hw j).1
  have h2u : ∀ n ∈ Finset.range (H + 1), ∀ w ∈ triadicIndexBox d n, ∀ j : Fin d,
    ...
    (h6a_integrableOn_optimizerField_respCoeffMinus_box (respGrid jStar F) hgrid t F a u n w
      hw j).2
  obtain ⟨V, hV⟩ :=
    h6a_recentHead_actual_of_analytic P γ hγ jStar H F t e hgrid a u hu hE hM0 hgood h1u h2u
  refine hV (fun n _ w _ j =>
      (h6a_integrableOn_optimizerField_respCoeffMinus_at (respGrid jStar F) hgrid
  ...
  · exact h6a_headCell_of_bridge P γ jStar F t hgrid a n u
      (fun w => (V n w).toAHarmonicFunctionMeanZero.toAHarmonicFunction) hm hEmean hE hM0 hbdd
  · exact h6a_headEnergy_carrier_minus P jStar F t e hgrid a n u hu
