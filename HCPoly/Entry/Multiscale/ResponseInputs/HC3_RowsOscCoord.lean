import HCPoly.Entry.Multiscale.ResponseInputs.HC3_RowsOscHalfCarPlus
...
import HCPoly.Entry.Multiscale.ResponseInputs.HC3_DirectLoadAllY
...
of one coordinate, and the Fenchel probe of `HC3_RowsCrossInt` makes the unweighted cell means
integrable on every aligned cell.  The source-load series converges at every dual vector
(`HC3_DirectLoadAllY`), so the `2d` instantiations are legitimate.
-/
...
private theorem vecDot_zero_left' {d : ℕ} (v : Vec d) : vecDot (0 : Vec d) v = 0 := by
  simp [vecDot]
...
theorem integrable_cutoffMeanDefect_coords_respCoeffMinus {d : ℕ} [NeZero d]
  ...
  have hmean1 : ∀ (m : ℕ), ∀ W ∈ triadicIndexBox d m, ∀ i : Fin d, Integrable (fun a =>
    ...
    rw [vecDot_single_left, vecDot_zero_left', add_zero]
