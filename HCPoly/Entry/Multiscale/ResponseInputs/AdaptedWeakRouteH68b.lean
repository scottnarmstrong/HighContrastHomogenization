...
private theorem b179_blockMat_eq_of_full {A B : BlockMat d}
...
theorem b179_blockCongr_blockSub (G A B : BlockMat d) :
    blockCongr G (blockSub A B) = blockSub (blockCongr G A) (blockCongr G B) := by
  refine b179_blockMat_eq_of_full ?_
  rw [blockCongr, toFullBlockMat_ofFullBlockMat, b130_toFullBlockMat_blockSub,
    b130_toFullBlockMat_blockSub, blockCongr, blockCongr, toFullBlockMat_ofFullBlockMat,
    toFullBlockMat_ofFullBlockMat, Matrix.mul_sub, Matrix.sub_mul]

/-- `A - B = (A - I) - (B - I)`. -/
private theorem b179_blockSub_shift_identity (A B : BlockMat d) :
    blockSub A B = blockSub (blockSub A (Book.Ch02.blockIdentity d))
      (blockSub B (Book.Ch02.blockIdentity d)) := by
  refine b179_blockMat_eq_of_full ?_
  rw [b130_toFullBlockMat_blockSub, b130_toFullBlockMat_blockSub, b130_toFullBlockMat_blockSub,
    b130_toFullBlockMat_blockSub]
  abel
...
`h68_weighted_blockOpNorm_le_respAllScaleAbs`, the depth-`n` one after multiplying by
`3 ^ (ρ n)` and the depth-`0` one at `z = 0`, where `triadicIndexBox d 0 = {0}` and
`adaptedCellAtCenter q t 0 = adaptedCell q t`. -/
theorem h68_blockOpNorm_normalizedBlock_sub_le (hd : 2 ≤ d) (γ : ℝ)
    (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
    ...
    ∀ᵐ a ∂P, ∀ (n : ℕ), ∀ w ∈ triadicIndexBox d n,
      blockOpNorm (normalizedBlock
        ...
        ≤ ((3 : ℝ) ^ (respRho γ * (n : ℝ)) + 1) * respAllScaleAbs P γ jStar F t a := by
  filter_upwards
    [h68_weighted_blockOpNorm_le_respAllScaleAbs hd γ hγ P E Ψ Kg Src hstat hdag jStar hj F hm t]
    with a hkey
  intro n w hw
  ...
  have hn := hkey n w hw
  have h0 := hkey 0 0 (b130_zero_mem_triadicIndexBox 0)
  have hw0 : (0 : ℝ) < (3 : ℝ) ^ (-(respRho γ * (n : ℝ))) := Real.rpow_pos_of_pos (by norm_num) _
  have hwmul : (3 : ℝ) ^ (respRho γ * (n : ℝ)) * (3 : ℝ) ^ (-(respRho γ * (n : ℝ))) = 1 := by
    rw [← Real.rpow_add (by norm_num)]
    simp
  have hwpos : (0 : ℝ) < (3 : ℝ) ^ (respRho γ * (n : ℝ)) := Real.rpow_pos_of_pos (by norm_num) _
  -- depth-`n` term
  have hn' : blockOpNorm (blockSub (normalizedBlock
      (coarseBlock (adaptedCellAtCenter q (t - (n : ℤ)) w) a) Et) (Book.Ch02.blockIdentity d))
      ≤ (3 : ℝ) ^ (respRho γ * (n : ℝ)) * Ab := by
    have := mul_le_mul_of_nonneg_left hn hwpos.le
    calc blockOpNorm (blockSub (normalizedBlock
            (coarseBlock (adaptedCellAtCenter q (t - (n : ℤ)) w) a) Et) (Book.Ch02.blockIdentity d))
        = (3 : ℝ) ^ (respRho γ * (n : ℝ)) * ((3 : ℝ) ^ (-(respRho γ * (n : ℝ))) *
  ...
  have h0' : blockOpNorm (blockSub (normalizedBlock
    ...
    have hp : (3 : ℝ) ^ (-(respRho γ * ((0 : ℕ) : ℝ))) = 1 := by
  ...
  have hsplit : normalizedBlock
      ...
      blockSub
        ...
        (blockSub (normalizedBlock (coarseBlock (HighContrast.adaptedCell q t) a) Et)
          (Book.Ch02.blockIdentity d)) := by
    rw [Annealed.normalizedBlock_blockSub, ← b179_blockSub_shift_identity]
  rw [hsplit, blockOpNorm, b130_toFullBlockMat_blockSub]
  ...
  nlinarith [htri, hn', h0']


/-- `Ehat^+ = D Ehat^- D` is the congruence of `E_t` by `respGPlus = respG · D`
(`AdaptedDefs.lean`, `blockCongr_blockCongr`), i.e. the plus sign is the same
picture with the same congruence that `h68_coarseBlockMatrix_respCoeffPlus_at` produces. -/
theorem b179_respEhatPlus_eq_blockCongr (P : Measure (CoeffSpace d)) (jStar : ℕ)
    (F : BlockMat d) (t : ℤ) :
    respEhatPlus P jStar F t = blockCongr (h68_respGPlus F) (respMean P jStar F t) := by
  rw [respEhatPlus, blockAdjoint, respEhatMinus, blockCongr_blockCongr, h68_respGPlus]


/-- The block shear `⟨1, 0, c, 1⟩` in `fromBlocks` form. -/
private theorem b179_shear_full (c : Mat d) :
...
private theorem b179_isUnit_respG (F : BlockMat d) : IsUnit (toFullBlockMat (respG F)) := by
  refine (Matrix.isUnit_iff_isUnit_det _).2
    (Matrix.isUnit_det_of_right_inverse
      (B := toFullBlockMat (⟨1, 0, -(respg F), 1⟩ : BlockMat d)) ?_)
  rw [respG, b179_shear_full, b179_shear_full, Matrix.fromBlocks_multiply]
  simp only [one_mul, mul_one, zero_mul, mul_zero, add_zero, zero_add, add_neg_cancel,
    Matrix.fromBlocks_one]

/-- `D = diag(Id, -Id)` in `fromBlocks` form. -/
private theorem b179_blockD_full :
    toFullBlockMat (blockD d) = Matrix.fromBlocks (1 : Mat d) 0 0 (-1) := by
  ext (i | i) (j | j) <;> rfl

/-- `D` is a unit: it is its own inverse. -/
private theorem b179_isUnit_blockD : IsUnit (toFullBlockMat (blockD d)) := by
  refine (Matrix.isUnit_iff_isUnit_det _).2
    (Matrix.isUnit_det_of_right_inverse (B := toFullBlockMat (blockD d)) ?_)
  rw [b179_blockD_full, Matrix.fromBlocks_multiply]
  simp only [zero_mul, mul_zero, add_zero, zero_add, neg_mul_neg, Matrix.fromBlocks_one,
    Matrix.one_mul]

/-- `respGPlus = respG · D` is a unit. -/
private theorem b179_isUnit_respGPlus (F : BlockMat d) :
    IsUnit (toFullBlockMat (h68_respGPlus F)) := by
  rw [h68_respGPlus, toFullBlockMat_ofFullBlockMat]
  exact (b179_isUnit_respG F).mul b179_isUnit_blockD

/-- **Congruence by a unit preserves positive definiteness.**  `Mathlib`'s
`Matrix.PosDef.conjTranspose_mul_mul_same` through `Matrix.mulVec_injective_of_isUnit`, with
`ᴴ = ᵀ` over `ℝ` — the same route as `HCPoly/Entry/Analysis/ReferenceComparison.lean`.  This is the
lemma the tree was missing, and it discharges the `hEhat` input of the η-kernel bounds. -/
theorem h68_blockCongr_posDef {G E : BlockMat d} (hG : IsUnit (toFullBlockMat G))
...
theorem h68_respEhatMinus_posDef (hd : 2 ≤ d) (γ : ℝ)
    (P : Measure (CoeffSpace d)) [IsProbabilityMeasure P]
    (E : BlockMat d) (Ψ : ℝ → ℝ) (Kg : ℝ) (Src : CoeffSpace d → ℝ)
    (hstat : IsStationaryLaw P) (hdag : CoarseEllipticityDagger P γ E Ψ Kg Src)
    (jStar : ℕ) (hj : 2 * d ≤ 3 ^ jStar) (F : BlockMat d)
...
theorem h68_respEhatPlus_posDef (hd : 2 ≤ d) (γ : ℝ)
  ...
  rw [b179_respEhatPlus_eq_blockCongr]
  exact h68_blockCongr_posDef (b179_isUnit_respGPlus F)
    (Annealed.adaptedMean_posDef d hd P γ E Ψ Kg Src hstat hdag jStar hj (explicitCanonicalMetric F)
      hm t)
...
(`h68_coarseBlockMatrix_respCoeffMinus_at`), congruence does not increase the
normalized operator norm (`h68_blockOpNorm_normalizedBlock_blockCongr_le`), and in the
`E_t`-picture the defect splits as `(N_{n,w} - I) - (N_0 - I)`
(`h68_blockOpNorm_normalizedBlock_sub_le`).

Positive definiteness of `Ehat^-` is DISCHARGED here, not assumed:
`h68_respEhatMinus_posDef` above, from `h68_blockCongr_posDef` and the unit block shear
`respG F = ⟨1, 0, respg F, 1⟩` (`AdaptedDefs.lean`). -/
theorem h68_recentDefect_le_respAllScaleAbs_minus (hd : 2 ≤ d) (γ : ℝ)
    (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
    (P : Measure (CoeffSpace d)) [IsProbabilityMeasure P]
    (E : BlockMat d) (Ψ : ℝ → ℝ) (Kg : ℝ) (Src : CoeffSpace d → ℝ)
    (hstat : IsStationaryLaw P) (hdag : CoarseEllipticityDagger P γ E Ψ Kg Src)
  ...
  have hEhat : (toFullBlockMat (respEhatMinus P jStar F t)).PosDef :=
    h68_respEhatMinus_posDef hd γ P E Ψ Kg Src hstat hdag jStar hj F hm t
  filter_upwards
    [h68_blockOpNorm_normalizedBlock_sub_le hd γ hγ P E Ψ Kg Src hstat hdag jStar hj F hm t]
    with a ha
  intro n w hw
  ...
  have hrw : recentDefect q t n w (respEhatMinus P jStar F t) (respCoeffMinus F a) =
    ...
    rw [recentDefect, recentDefectBlock, hEhd, blockOpNorm,
      h68_coarseBlockMatrix_respCoeffMinus_at hq (t - (n : ℤ)) w F a, hcell,
      h68_coarseBlockMatrix_respCoeffMinus_at hq t 0 F a, ← b179_blockCongr_blockSub, ← hcell]
  rw [hrw]
  exact le_trans
    (h68_blockOpNorm_normalizedBlock_blockCongr_le (respG F) _ _ hEt (hEhd ▸ hEhat))
    (ha n w hw)


/-- **THE η-KERNEL POINTWISE BOUND, plus sign.**  The twin of
`h68_recentDefect_le_respAllScaleAbs_minus`, with `h68_respGPlus` in place of `respG`. -/
theorem h68_recentDefect_le_respAllScaleAbs_plus (hd : 2 ≤ d) (γ : ℝ)
    (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
    (P : Measure (CoeffSpace d)) [IsProbabilityMeasure P]
    (E : BlockMat d) (Ψ : ℝ → ℝ) (Kg : ℝ) (Src : CoeffSpace d → ℝ)
    (hstat : IsStationaryLaw P) (hdag : CoarseEllipticityDagger P γ E Ψ Kg Src)
  ...
  have hEhat : (toFullBlockMat (respEhatPlus P jStar F t)).PosDef :=
    h68_respEhatPlus_posDef hd γ P E Ψ Kg Src hstat hdag jStar hj F hm t
  filter_upwards
    [h68_blockOpNorm_normalizedBlock_sub_le hd γ hγ P E Ψ Kg Src hstat hdag jStar hj F hm t]
    with a ha
  intro n w hw
  set q := respGrid jStar F with hqd
  have hcell : HighContrast.adaptedCell q t = adaptedCellAtCenter q t 0 := (b130_adaptedCellAtCenter_zero q t).symm
  have hEhd := b179_respEhatPlus_eq_blockCongr P jStar F t
  have hrw : recentDefect q t n w (respEhatPlus P jStar F t) (respCoeffPlus F a) =
      blockOpNorm (normalizedBlock
        (blockCongr (h68_respGPlus F)
          (blockSub (coarseBlock (adaptedCellAtCenter q (t - (n : ℤ)) w) a)
            (coarseBlock (HighContrast.adaptedCell q t) a)))
        (blockCongr (h68_respGPlus F) (respMean P jStar F t))) := by
    rw [recentDefect, recentDefectBlock, hEhd, blockOpNorm,
      h68_coarseBlockMatrix_respCoeffPlus_at hq (t - (n : ℤ)) w F a, hcell,
      h68_coarseBlockMatrix_respCoeffPlus_at hq t 0 F a, ← b179_blockCongr_blockSub, ← hcell]
  rw [hrw]
  exact le_trans
    (h68_blockOpNorm_normalizedBlock_blockCongr_le (h68_respGPlus F) _ _ hEt (hEhd ▸ hEhat))
    (ha n w hw)
...
theorem h68_respAllScaleMax_le_respAllScaleAbs_ae (hd : 2 ≤ d) (γ : ℝ)
    (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
    (P : Measure (CoeffSpace d)) [IsProbabilityMeasure P]
    (E : BlockMat d) (Ψ : ℝ → ℝ) (Kg : ℝ) (Src : CoeffSpace d → ℝ)
    (hstat : IsStationaryLaw P) (hdag : CoarseEllipticityDagger P γ E Ψ Kg Src)
  ...
  filter_upwards [b179_pathwise_envelope_abs hd γ hγ P E Ψ Kg Src hstat hdag jStar hj F hm t]
    with a hbdd
  exact h68_respAllScaleMax_le_respAllScaleAbs P γ jStar F t a hbdd
...
theorem h68_integral_sq_le_rpow {α : Type*} [MeasurableSpace α] (P : Measure α)
  ...
  · -- the generic case
    have hpow : ∀ a, f a ^ 2 * I ^ (1 - θ) ≤ θ * f a ^ Q + (1 - θ) * I := by
      intro a
      have hgm := Real.geom_mean_le_arith_mean2_weighted hθ0.le (by linarith : (0:ℝ) ≤ 1 - θ)
...
`FullBlockMat d`. -/
private theorem b179_measurable_blockOpNorm {α : Type*} [MeasurableSpace α]
...
theorem h68_respAllScaleAbs_measurable [NeZero d]
  ...
  exact (b179_measurable_blockOpNorm _ fun α β =>
    h67_measurable_normalized_entry hq _ _ _ α β).const_mul _

/-- `0 ≤ respAllScaleAbs`: every member of the defining set is nonnegative, and `Real.sSup` of
the empty set is `0`. -/
theorem h68_respAllScaleAbs_nonneg (P : Measure (CoeffSpace d)) (γ : ℝ) (jStar : ℕ)
    (F : BlockMat d) (t : ℤ) (a : CoeffSpace d) : 0 ≤ respAllScaleAbs P γ jStar F t a := by
  rw [respAllScaleAbs]
...
(`AdaptedSwarm.lean`) -- but the first had none, since it is produced above only
...
Route, the one-sided twin with the two-sided ingredients: `h68_respAllScaleAbs_measurable` for the
...
set satisfiable), `h5abs_pathwise_bound` in place of `h5rc_pathwise_bound`, and
`h5abs_add4_pow_le` in place of `h5rc_add3_pow_le` because the two-sided pathwise bound carries
...
theorem respAllScaleAbs_aestronglyMeasurable_integrable (d : ℕ) (hd : 2 ≤ d) (γ : ℝ)
  ...
  have hMmeas : Measurable (respAllScaleAbs P γ jStar F t) :=
    h68_respAllScaleAbs_measurable P γ jStar F hq t
  ...
  have hkey : ∀ᵐ a ∂P, respAllScaleAbs P γ jStar F t a ^ bigQ d γ ≤
    ...
    have hMle : respAllScaleAbs P γ jStar F t a ≤
        F0 a ^ ((bigQ d γ : ℝ)⁻¹) +
          ...
          η ^ ((1 : ℝ) / (bigQ d γ : ℝ)) / 1 :=
      h5abs_pathwise_bound d hd γ hγ P E Ψ Kg Src raw.stat raw.unit raw.ell jStar raw.hj F hm
    ...
    calc respAllScaleAbs P γ jStar F t a ^ bigQ d γ
        ≤ (F0 a ^ ((bigQ d γ : ℝ)⁻¹) +
          ...
          pow_le_pow_left₀ (h68_respAllScaleAbs_nonneg P γ jStar F t a) hMle _
  ...
  rw [Real.norm_eq_abs,
    abs_of_nonneg (pow_nonneg (h68_respAllScaleAbs_nonneg P γ jStar F t a) _)]
  exact ha
...
`h68_integral_sq_le_rpow` (Jensen) gives the exponent.  The last analytic step. -/
theorem b179_conjuncts_of_pointwise (P : Measure (CoeffSpace d))
  ...
  have hsq_le : ∀ a, A a ^ 2 ≤ 1 + A a ^ Q := by
    ...
    · have : A a ^ 2 ≤ 1 := by nlinarith [hA0 a]
  ...
  have hstep2 : ∫ a, A a ^ 2 ∂P ≤ (∫ a, A a ^ Q ∂P) ^ ((2 : ℝ) / (Q : ℝ)) :=
    h68_integral_sq_le_rpow P Q hQ A hA0 hA2 hAint
  have hQ0 : (0 : ℝ) < (Q : ℝ) := by
    have : (0 : ℕ) < Q := by omega
    exact_mod_cast this
