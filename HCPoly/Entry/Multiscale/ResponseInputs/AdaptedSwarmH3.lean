import HCPoly.Entry.Multiscale.ResponseInputs.AdaptedSwarmH3Schur

/-!
# AdaptedSwarm, part 6 of 11
...
variable {d : ℕ}
private theorem h3x_respTau_le [NeZero d] (hd : 2 ≤ d) (P : Measure (CoeffSpace d))
  ...
  obtain ⟨hE5, hE6⟩ := h3x_respEJ_le hd P jStar F s t e he hq hg hsymm hposd hintT hkts
  have hrnn : (0 : ℝ) ≤ respRatio P jStar F s t - 1 := by linarith only [hr1]
  constructor
  · have hEJm := h3_respEJMinus_eq_final P jStar F t e he hq hg hM hintT
    have htau := h3_respTauMinus_eq P jStar F s t e
      (fun a => h3_respJ_eq_respCoeffMinus (respGrid jStar F) hq s F hg a _ _)
      (h3_integrable_respCoeffMinus P jStar F s hq hg hintS)
      (h3_annealedBlockOf_respCoeffMinus P jStar F s hq hg hintS) hEJm
      (h3_vecDot_respP_respqMinus P jStar F t e he hM hg)
    have hcmp2 := h3x_ehatMinus_scale_cmp P jStar F s t (respxMinus P jStar F t e)
  ...
  · have hEJp := h3_respEJPlus_eq_final P jStar F t e he hq hg hM hintT
    have htau := h3_respTauPlus_eq P jStar F s t e
      (fun a => h3_respJ_eq_respCoeffPlus (respGrid jStar F) hq s F hg a _ _)
      (h3_integrable_respCoeffPlus P jStar F s hq hg hintS)
      (h3_annealedBlockOf_respCoeffPlus P jStar F s hq hg hintS) hEJp
      (h3_vecDot_respP_respqPlus P jStar F t e he hM hg)
    have hcmp2 := h3x_ehatPlus_scale_cmp P jStar F s t (respxPlus P jStar F t e)
      (respRatio P jStar F s t) hcmp
...
theorem response_energy_and_defect (d : ℕ) (_hd : 2 ≤ d) (γ : ℝ) (_hγ : γ ∈ Set.Ico (0 : ℝ) 1)
  ...
  have hFpd : (toFullBlockMat F).PosDef := full_posDef raw.symm raw.pos
  ...
  have hintT : HasIntegrableCoarseBlock P (respCell jStar F t) :=
    h3_hasIntegrableCoarseBlock_respCell d _hd γ P E Ψ Kg Src raw.stat raw.ell jStar raw.hj F hm t
  have hintS : HasIntegrableCoarseBlock P (respCell jStar F s) :=
    h3_hasIntegrableCoarseBlock_respCell d _hd γ P E Ψ Kg Src raw.stat raw.ell jStar raw.hj F hm s
  ...
  have hord : BlockMatLoewnerLE (respMean P jStar F t) (respMean P jStar F s) :=
    h3_respMean_order_of_raw d _hd γ _hγ S ε σ Cglob Cprof Csrc Bresp hε hσ H P E Ψ Kg Src B
      jStar F s t raw
  ...
  have hts : toFullBlockMat (respMean P jStar F t) ≤ toFullBlockMat (respMean P jStar F s) :=
    full_le_of_block hEtpd.isHermitian hEspd.isHermitian hord
  ...
  have hcmp : ∀ Y : BlockVec d, blockVecDot Y (blockMatVecMul (respMean P jStar F s) Y)
      ≤ respRatio P jStar F s t * blockVecDot Y (blockMatVecMul (respMean P jStar F t) Y) :=
    h3x_respMean_scale_cmp P jStar F s t (respRatio P jStar F s t) hEsr
  have hk1s : 1 ≤ respKappa P jStar F s := le_trans hk1 hkts
  have hsqnn : (0 : ℝ) ≤ Real.sqrt (respKappa P jStar F s) := Real.sqrt_nonneg _
  obtain ⟨hE5, hE6⟩ := h3x_respEJ_le _hd P jStar F s t e he hq hg hsymm hposd hintT hkts
  obtain ⟨hE9, hE10⟩ := h3x_respTau_le _hd P jStar F s t e he hq hg hsymm hposd hintT hintS
    hkts hk1s hr1 hcmp
  exact ⟨h3_zero_le_respEJMinus P jStar F t e hq hg,
    h3_zero_le_respEJPlus P jStar F t e hq hg,
    ...
    h3_zero_le_respTauMinus_final P jStar F s t e he hq hg hM hord hintT hintS,
    h3_zero_le_respTauPlus_final P jStar F s t e he hq hg hM hord hintT hintS,
    hE9, hE10⟩
...
/-- `toFullBlockMat` is additive on `blockSub`; the flat representation of the difference of
two doubled blocks.  Auxiliary to the Step 5 split (`p.response.transfer`). -/
theorem h5_toFullBlockMat_blockSub (A B : BlockMat d) :
...
theorem h5_toFullBlockMat_blockScale_identity (c : ℝ) :
...
(`p.response.transfer`). -/
theorem h5_qform_add_of_full (C A B : BlockMat d)
...
private theorem allScale_full_split (A Ak At : BlockMat d) :
  ...
  simp only [h5_toFullBlockMat_blockSub, normalizedBlock, toFullBlockMat_ofFullBlockMat,
    Matrix.mul_sub, Matrix.sub_mul]
  abel

/-- The flat representation of `(c_V + c_P) I_{2d}` splits additively
(`p.response.transfer`). -/
private theorem h5_full_scale_add (cv cm : ℝ) :
  ...
  simp only [h5_toFullBlockMat_blockScale_identity, add_smul]
...
theorem allScale_specBound_split_le (A Ak At : BlockMat d) (cv cm : ℝ)
  ...
  have hv := hV X
  have hm := hM X
  rw [h5_qform_add_of_full _ _ _ (allScale_full_split A Ak At) X,
    h5_qform_add_of_full _ _ _ (h5_full_scale_add cv cm) X]
  linarith only [hv, hm]
...
(`h5_mean_part_le_determinantDrift`) and supply the two ingredients needed for the fluctuation half (the
index-box inclusion `h5_triadicIndexBox_center_mem` and the weight trade
`h5_rhoMax_lt_respRho`).
...
theorem h5_loewner_le_blockOpNorm (N : BlockMat d)
...
private theorem h5_blockSpecBound_le_blockTrace (N : BlockMat d)
    (hN : (toFullBlockMat N).PosSemidef) :
...
semidefinite. -/
theorem h5_normalizedBlock_gap_posSemidef (A B R : BlockMat d)
...
(`h5_normalizedBlock_gap_posSemidef`); `h5_blockSpecBound_le_blockTrace` replaces the spectral
bound by `tr(P^q_{k,t} - I_{2d})`; `Annealed.normalizedMean_self` gives `P^q_{t,t} = I_{2d}`, so
`h5_telescope_Icc` writes that trace as `sum_{j=k+1}^{t} tr(P^q_{j-1,t} - P^q_{j,t})`; every
...
`0 <= t - j <= t - k`. -/
theorem h5_mean_part_le_determinantDrift (d : ℕ) (hd : 2 ≤ d) (γ : ℝ)
    (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
    ...
    (3 : ℝ) ^ (-(respRho γ * ((t : ℝ) - (k : ℝ)))) *
        blockSpecBound (blockSub (normalizedMean P (Geometry.explicitRoundedGrid jStar mt) k t)
  ...
  have hstep1 : blockSpecBound (blockSub (normalizedMean P q k t) (Book.Ch02.blockIdentity d))
      ≤ ∑ j ∈ Finset.Icc (k + 1) t, T j := by
    have hpsd : (toFullBlockMat (blockSub (normalizedMean P q k t)
        (Book.Ch02.blockIdentity d))).PosSemidef := by
      rw [← hI]; exact hgap k t hk hkt
    refine (h5_blockSpecBound_le_blockTrace _ hpsd).trans_eq ?_
  ...
  have hrho : (1 : ℝ) / 2 ≤ respRho γ := by
    unfold respRho; linarith only [hγ.1]
  have hw : ∀ j ∈ Finset.Icc (k + 1) t,
    ...
    have s2 : (1 / 8 : ℝ) * ((t : ℝ) - (j : ℝ)) ≤ (1 / 8 : ℝ) * ((t : ℝ) - (k : ℝ)) := by
      linarith only [h2]
    have s3 : (1 / 8 : ℝ) * ((t : ℝ) - (k : ℝ)) ≤ respRho γ * ((t : ℝ) - (k : ℝ)) :=
  ...
  calc
    (3 : ℝ) ^ (-(respRho γ * ((t : ℝ) - (k : ℝ)))) *
        blockSpecBound (blockSub (normalizedMean P q k t) (Book.Ch02.blockIdentity d))
        ≤ (3 : ℝ) ^ (-(respRho γ * ((t : ℝ) - (k : ℝ)))) * ∑ j ∈ Finset.Icc (k + 1) t, T j :=
...
`HCPoly/Entry/Multiscale/ResponseInputs/HC2_WeakSeminorm.lean`, which `AdaptedSwarm` does not
import. -/
private theorem h5_standardCell_subset_centeredCube (t : ℤ) (n : ℕ)
    {w : Fin d → ℤ} (hw : w ∈ triadicIndexBox d n) :
    HighContrast.standardCell d (t - (n : ℤ)) w ⊆ HighContrast.centeredCube d t := by
  intro x hx
  rw [Geometry.mem_standardCell_iff] at hx
...
/-- The adapted-cell form of the previous inclusion: the depth-`n` aligned adapted cell at an
index of `triadicIndexBox d n` lies in `U_t` (`p.response.transfer`). -/
theorem h5_adaptedCellAtCenter_subset_adaptedCell (q : Mat d) (t : ℤ) (n : ℕ)
    {w : Fin d → ℤ} (hw : w ∈ triadicIndexBox d n) :
    adaptedCellAtCenter q (t - (n : ℤ)) w ⊆ HighContrast.adaptedCell q t := by
  rw [Geometry.adaptedCellAtCenter_eq_affine_standardCell]
  rintro x ⟨v, hv, rfl⟩
  exact ⟨v, h5_standardCell_subset_centeredCube t n hw hv, rfl⟩

/-- The centre of an aligned adapted cell lies in that cell (the cell is an open cube around
its centre).  `p.response.transfer`. -/
private theorem h5_adaptedCellCenter_mem_adaptedCellAtCenter (q : Mat d) (j : ℤ) (w : Fin d → ℤ) :
...
`fluctuationHistory`. -/
private theorem h5_triadicIndexBox_center_mem (q : Mat d) (t : ℤ) (n : ℕ)
    {w : Fin d → ℤ} (hw : w ∈ triadicIndexBox d n) :
    adaptedCellCenter q (t - (n : ℤ)) w ∈
      adaptedLatticeAtScale q (t - (n : ℤ)) ∩ HighContrast.adaptedCell q t :=
  ⟨⟨w, rfl⟩, h5_adaptedCellAtCenter_subset_adaptedCell q t n hw
    (h5_adaptedCellCenter_mem_adaptedCellAtCenter q (t - (n : ℤ)) w)⟩
...
theorem h5_rhoMax_lt_respRho (d : ℕ) (hd : 2 ≤ d) (γ : ℝ)
    (hγ : γ ∈ Set.Ico (0 : ℝ) 1) : rhoMax d γ < respRho γ := by
  have hq : 0 < (bigQ d γ : ℝ) := bigQ_real_pos d hd γ hγ
...
They cite the `private` helpers `h5_triadicIndexBox_center_mem` and
`h5_rhoMax_lt_respRho`. -/
...
`3^j q z` lies in `3^j q Z^d ∩ U_t` by `h5_triadicIndexBox_center_mem`; and the weight is traded
by `h5_rhoMax_lt_respRho`. -/
theorem h5ra_scale_index_le (d : ℕ) (hd : 2 ≤ d) (γ : ℝ)
    (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
    (P : Measure (CoeffSpace d)) [IsProbabilityMeasure P]
    (E : BlockMat d) (Ψ : ℝ → ℝ) (K : ℝ) (Src : CoeffSpace d → ℝ)
    (hstat : IsStationaryLaw P) (hdag : CoarseEllipticityDagger P γ E Ψ K Src)
        ...
        (3 : ℝ) ^ (-(bigQ d γ : ℝ) * rhoMax d γ * ((t : ℝ) - (j : ℝ))) *
          ⨆ y ∈ adaptedLatticeAtScale (Geometry.explicitRoundedGrid jStar mt) j ∩
            ...
            blockOpNorm (normalizedFluctuation P (Geometry.explicitRoundedGrid jStar mt) j t y a) ^
              bigQ d γ := by
  have hdom := (oneGrid_fluctuation_integrable_dominate d hd γ hγ P E Ψ K Src hstat hdag
  ...
  have hmem := h5_triadicIndexBox_center_mem (Geometry.explicitRoundedGrid jStar mt) t n' hz
  ...
  refine Real.rpow_le_rpow_of_exponent_le (by norm_num) ?_
  have hQ : (0 : ℝ) < (bigQ d γ : ℝ) := bigQ_real_pos d hd γ hγ
