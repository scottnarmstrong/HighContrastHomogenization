import HCPoly.Entry.Multiscale.ResponseInputs.HC3_CutoffBesovPathwise
import HCPoly.Entry.Multiscale.ResponseInputs.HC3_CutoffWeakGlue
import HCPoly.Entry.Multiscale.ResponseInputs.HC3_CutoffSubcellLoewner
...
`X ⬝ X = |X.1| ^ 2 + |X.2| ^ 2 ≥ 0`. -/
private theorem hc3_dom_blockVecDot_self_nonneg {d : ℕ} (X : BlockVec d) :
    0 ≤ blockVecDot X X := by
  have h1 := vecNormSq_nonneg X.1
  have h2 := vecNormSq_nonneg X.2
  simp only [vecNormSq] at h1 h2
...
private theorem hc3_dom_qform_self_le_opNorm_sq {d : ℕ} (A : BlockMat d) :
...
private theorem hc3_dom_qform_le_opNorm_of_posSemidef {d : ℕ} {A : BlockMat d}
...
private theorem ae_besovSeminorm_sq_le_envelope_of_recentring {d : ℕ} [NeZero d]
  ...
  have hρ0 : 0 ≤ respRho γ := by
    simp only [respRho]; linarith [hγ.1]
  have hρ1 : respRho γ < 1 := by
    simp only [respRho]; linarith [hγ.2]
  ...
  have hEbound : ∀ X : BlockVec d,
    ...
    intro X; exact hc3_dom_qform_le_opNorm_of_posSemidef hEt_psd X
  have hGbound : ∀ X : BlockVec d,
      blockVecDot (blockMatVecMul G X) (blockMatVecMul G X) ≤ kG * blockVecDot X X := by
    intro X; exact hc3_dom_qform_self_le_opNorm_sq G X
  have hSbound : ∀ X : BlockVec d,
    ...
    intro X; exact hc3_dom_qform_self_le_opNorm_sq (blockSqrt (respM0 F)) X
  -- the final constant: the pathwise gain, the metric bound, and the envelope coefficients
  let Cst : ℝ := ((1 - (3 : ℝ) ^ ((respRho γ - 1) / 2))⁻¹) ^ 2 * (3 : ℝ) ^ (t : ℝ) * kS
  ...
  have hCst : 0 ≤ Cst := by
    ...
    · have h1 : 0 ≤ kE * kG := mul_nonneg hkE hkG
      have h2 : 0 ≤ blockVecDot Y Y := hc3_dom_blockVecDot_self_nonneg Y
      nlinarith [h1, h2]
  refine ⟨Cst, hCst, ?_⟩
  ...
  have hK₀ : 0 < K₀ := by
    have hnonneg : 0 ≤ kE * kG * (1 + respAllScaleMax P γ jStar F t a) :=
      mul_nonneg (mul_nonneg hkE hkG) (by linarith [hMnn])
    simp only [K₀]
    linarith
  have hBf : ∀ (n : ℕ), ∀ w ∈ triadicIndexBox d n, ∀ X : BlockVec d,
      blockVecDot X (blockMatVecMul (ofFullBlockMat (toFullBlockMat
        ...
        ≤ (K₀ * (3 : ℝ) ^ (respRho γ * (n : ℝ))) * blockVecDot X X := by
    intro n w hw X
    have hVU : adaptedCellAtCenter (respGrid jStar F) (t - (n : ℤ)) w ⊆ respCell jStar F t :=
      adaptedCellAtCenter_subset_adaptedCell (respGrid jStar F) t n hw
    ...
    have hLo' := hLo n w hw
    have hc : 0 ≤ 1 + (3 : ℝ) ^ (respRho γ * (n : ℝ)) * respAllScaleMax P γ jStar F t a := by
      have hp : 0 ≤ (3 : ℝ) ^ (respRho γ * (n : ℝ)) * respAllScaleMax P γ jStar F t a :=
    ...
    have hqA : ∀ X : BlockVec d, blockVecDot X (blockMatVecMul
        ...
        ≤ ((1 + (3 : ℝ) ^ (respRho γ * (n : ℝ)) * respAllScaleMax P γ jStar F t a) * kE)
            * blockVecDot X X :=
    ...
    have hqcon : ∀ X : BlockVec d, blockVecDot X (blockMatVecMul
        ...
        ≤ (((1 + (3 : ℝ) ^ (respRho γ * (n : ℝ)) * respAllScaleMax P γ jStar F t a) * kE) * kG)
            * blockVecDot X X :=
    ...
    have hp1 : (1 : ℝ) ≤ (3 : ℝ) ^ (respRho γ * (n : ℝ)) :=
      Real.one_le_rpow (by norm_num) (mul_nonneg hρ0 (Nat.cast_nonneg n))
    have hcoef : (((1 + (3 : ℝ) ^ (respRho γ * (n : ℝ))
            * respAllScaleMax P γ jStar F t a) * kE) * kG + 1)
        ≤ K₀ * (3 : ℝ) ^ (respRho γ * (n : ℝ)) := by
      have hkEG : 0 ≤ kE * kG := mul_nonneg hkE hkG
      have hkey : (kE * kG * (1 + respAllScaleMax P γ jStar F t a) + 1)
            * (3 : ℝ) ^ (respRho γ * (n : ℝ))
          - ((((1 + (3 : ℝ) ^ (respRho γ * (n : ℝ))
              * respAllScaleMax P γ jStar F t a) * kE) * kG) + 1)
          = (kE * kG + 1) * ((3 : ℝ) ^ (respRho γ * (n : ℝ)) - 1) := by ring
      have hnn : 0 ≤ (kE * kG + 1) * ((3 : ℝ) ^ (respRho γ * (n : ℝ)) - 1) :=
        mul_nonneg (by linarith [hkEG]) (by linarith [hp1])
      simp only [K₀]
      linarith [hkey, hnn]
    exact mul_le_mul_of_nonneg_right hcoef (hc3_dom_blockVecDot_self_nonneg X)
  ...
  have hpath := besovSeminorm_sq_optimizerState_le_of_scale_bounds (q := respGrid jStar F)
    hq t (respRho γ) hρ0 hρ1 hEll v (blockSqrt (respM0 F)) Y kS hkS hSbound K₀ hK₀ hBf hint hEnn
  ...
  have hinner : 2 * (2 * ((kE * kG * (1 + respAllScaleMax P γ jStar F t a) + 1)
    ...
    have hYnn : 0 ≤ blockVecDot Y Y := hc3_dom_blockVecDot_self_nonneg Y
    have hkEG : 0 ≤ kE * kG := mul_nonneg hkE hkG
    ...
    calc
      ...
      _ ≤ 4 * (kE * kG)
          ...
          have h2 : 4 * respJ (respGrid jStar F) t p q' (c a)
              ≤ 4 * ((1 + respAllScaleMax P γ jStar F t a)
                  * respJ (respGrid jStar F) t p q' (c a) + 1) := by linarith [hQJ]
  ...
  have hpath' : ((1 - (3 : ℝ) ^ ((respRho γ - 1) / 2))⁻¹) ^ 2
      ...
      ≤ Cst * ((1 + respAllScaleMax P γ jStar F t a)
          * respJ (respGrid jStar F) t p q' (c a) + 1) := by
    have hkey : ((1 - (3 : ℝ) ^ ((respRho γ - 1) / 2))⁻¹) ^ 2
          * ((3 : ℝ) ^ (t : ℝ) * (kS * (2 * (K₀ * volumeAverage (respCell jStar F t)
                (scalarVariationEnergyIntegrand f v)) + 2 * blockVecDot Y Y)))
        = ((1 - (3 : ℝ) ^ ((respRho γ - 1) / 2))⁻¹) ^ 2 * (3 : ℝ) ^ (t : ℝ) * kS
            * (2 * (2 * ((kE * kG * (1 + respAllScaleMax P γ jStar F t a) + 1)
...
theorem ae_besovSeminorm_sq_le_envelope_minus {d : ℕ} [NeZero d] (hd : 2 ≤ d) (γ : ℝ)
  ...
  exact ae_besovSeminorm_sq_le_envelope_of_recentring hd γ hγ P E Ψ Kg Src hstat hdag jStar hj F hm
    ...
    (fun a j w => h68_coarseBlockMatrix_respCoeffMinus_at hq j w F a) u hu
...
theorem ae_besovSeminorm_sq_le_envelope_plus {d : ℕ} [NeZero d] (hd : 2 ≤ d) (γ : ℝ)
  ...
  have hq : IsUnit (respGrid jStar F) := Geometry.isUnit_roundedGrid hj hm
  exact ae_besovSeminorm_sq_le_envelope_of_recentring hd γ hγ P E Ψ Kg Src hstat hdag jStar hj F hm
    t p q' Y (h68_respGPlus F) (respCoeffPlus F)
    (fun a => exists_elliptic_representative_respCell_respCoeffPlus hj hm t a)
    (fun a j w => h68_coarseBlockMatrix_respCoeffPlus_at hq j w F a) u hu
