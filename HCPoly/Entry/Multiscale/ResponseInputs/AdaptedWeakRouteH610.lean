import HCPoly.Entry.Multiscale.ResponseInputs.AdaptedWeakRouteH68c
...
`centered_metric_quadratic_le`; it lives in `RecentreCore.lean`, imported above.  What remains
...
(`h68_blockOpNorm_normalizedBlock_blockCongr_le`), `normalizedBlock` is linear
...
private theorem d124_normalizedDefect_le_respAllScaleAbs_minus (hd : 2 ≤ d) (γ : ℝ)
    (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
  ...
  rw [b130_adaptedCellAtCenter_zero] at h0
  ...
  have hAeq : coarseBlockMatrix (respCell jStar F t) (respCoeffMinus F a)
    ...
    have hx := h68_coarseBlockMatrix_respCoeffMinus_at hq t 0 F a
    rwa [b130_adaptedCellAtCenter_zero] at hx
  have hsub : blockSub (coarseBlockMatrix (respCell jStar F t) (respCoeffMinus F a))
    ...
    rw [hAeq, hEhd, ← b179_blockCongr_blockSub]
  have hcongr := h68_blockOpNorm_normalizedBlock_blockCongr_le (respG F)
...
private theorem d124_normalizedDefect_le_respAllScaleAbs_plus (hd : 2 ≤ d) (γ : ℝ)
    (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
    (P : Measure (CoeffSpace d)) [IsProbabilityMeasure P]
    (E : BlockMat d) (Ψ : ℝ → ℝ) (Kg : ℝ) (Src : CoeffSpace d → ℝ)
    (hstat : IsStationaryLaw P) (hdag : CoarseEllipticityDagger P γ E Ψ Kg Src)
  ...
  have hAeq : coarseBlockMatrix (respCell jStar F t) (respCoeffPlus F a)
      = blockCongr (h68_respGPlus F) (coarseBlock (respCell jStar F t) a) := by
    have hx := h68_coarseBlockMatrix_respCoeffPlus_at hq t 0 F a
    rwa [b130_adaptedCellAtCenter_zero] at hx
  have hsub : blockSub (coarseBlockMatrix (respCell jStar F t) (respCoeffPlus F a))
      ...
      = blockCongr (h68_respGPlus F)
          (blockSub (coarseBlock (respCell jStar F t) a) (respMean P jStar F t)) := by
    rw [hAeq, hEhd, ← b179_blockCongr_blockSub]
  have hcongr := h68_blockOpNorm_normalizedBlock_blockCongr_le (h68_respGPlus F)
    (blockSub (coarseBlock (respCell jStar F t) a) (respMean P jStar F t))
    (respMean P jStar F t) hEt (hEhd ▸ hEhat)
  have hsplit : normalizedBlock
      (blockSub (coarseBlock (respCell jStar F t) a) (respMean P jStar F t))
...
private theorem d124_measurable_recentre_sq {Ω : Type*} [MeasurableSpace Ω]
  ...
  have hflat : ∀ ω, toFullBlockVec (blockMatVecMul (blockSqrt (respM0 F))
    ...
    rw [blockResponseMean_sub_blockResponseMean, toFullBlockVec_blockMatVecMul,
      ...
      b130_toFullBlockMat_blockSub, Matrix.mulVec_mulVec, Matrix.mulVec_mulVec, hS,
      Matrix.mul_assoc]
  ...
  have hM : ∀ β : BlockCoord d, Measurable fun ω =>
    ...
    intro β
    have hx := h68_measurable_mul_mul S 1
...
`b179_conjuncts_of_pointwise` above: the majorant is `c · A²` rather than `(c · A)²`, because the
recentring bound is already stated on the SQUARE. -/
private theorem d124_conjuncts_of_pointwise (P : Measure (CoeffSpace d))
  ...
  have hsq_le : ∀ a, A a ^ 2 ≤ 1 + A a ^ Q := by
    ...
    · have h2 : A a ^ 2 ≤ 1 := by nlinarith [hA0 a]
...
private theorem d124_blockVecDot_self_nonneg (X : BlockVec d) : 0 ≤ blockVecDot X X := by
  rw [← dotProduct_toFullBlockVec]
  exact b130_dotProduct_self_nonneg _

/-- `0 ≤ x · Ehat x` for a positive semidefinite doubled block. -/
private theorem d124_qform_nonneg {Ehat : BlockMat d}
...
theorem integral_respRecentre_sq_le_minus (d : ℕ) (hd : 2 ≤ d) (γ : ℝ)
  ...
  have hEhat : (toFullBlockMat (respEhatMinus P jStar F t)).PosDef :=
    h68_respEhatMinus_posDef hd γ P E Ψ Kg Src raw.stat raw.ell jStar raw.hj F hm t
  ...
  have hmeas : AEStronglyMeasurable (fun a : CoeffSpace d =>
    ...
    rw [hfeq]
    refine d124_measurable_recentre_sq F (respEhatMinus P jStar F t) (respxMinus P jStar F t e)
      (fun a => coarseBlockMatrix (respCell jStar F t) (respCoeffMinus F a)) ?_
    intro ζ δ
    have hx := h68_measurable_coarseBlockMatrix_minus hq t 0 F ζ δ
    rwa [b130_adaptedCellAtCenter_zero] at hx
  have hK0 : (0 : ℝ) ≤ respK0SqMinus P jStar F t := b179_blockSpecBound_nonneg _
  have hL0 : (0 : ℝ) ≤ respLsqMinus P jStar F t e :=
    d124_qform_nonneg hEhat.posSemidef (respxMinus P jStar F t e)
  have hbound : ∀ᵐ a ∂P, blockVecDot (respRecentreMinus P jStar F t e a (u a))
    ...
    filter_upwards [d124_normalizedDefect_le_respAllScaleAbs_minus hd γ hγ P E Ψ Kg Src
      raw.stat raw.ell jStar raw.hj F hm t] with a ha
    have hpt := d124_recentre_sq_le_minus P jStar F t e hq hm hEhat a (u a) (hu a)
    ...
    have hA0 : (0 : ℝ) ≤ respAllScaleAbs P γ jStar F t a :=
      h68_respAllScaleAbs_nonneg P γ jStar F t a
    ...
    have hsq : NN ^ 2 ≤ AA ^ 2 := by nlinarith [ha, hn0, hA0]
    have hfin := mul_le_mul_of_nonneg_left hsq (mul_nonneg hK0 hL0)
    linarith [hfin]
  obtain ⟨hint, hle⟩ := d124_conjuncts_of_pointwise P (bigQ d γ) hQ2 Cm η hCm hη.1 _ _
    (h68_respAllScaleAbs_nonneg P γ jStar F t)
    (fun a => d124_blockVecDot_self_nonneg _) hmeas
    (((h68_respAllScaleAbs_measurable P γ jStar F hq t).pow_const 2).aestronglyMeasurable)
...
theorem integral_respRecentre_sq_le_plus (d : ℕ) (hd : 2 ≤ d) (γ : ℝ)
  ...
  have hq : IsUnit (respGrid jStar F) := Geometry.isUnit_roundedGrid raw.hj hm
  have hEhat : (toFullBlockMat (respEhatPlus P jStar F t)).PosDef :=
    h68_respEhatPlus_posDef hd γ P E Ψ Kg Src raw.stat raw.ell jStar raw.hj F hm t
  have hQ2 : 2 ≤ bigQ d γ := bigQ_two_le d hd γ hγ
  -- The mean identity eliminates the optimizer family `u` from `c_a`, which is what makes conjunct 1 provable.
  have hcell : ∀ a : CoeffSpace d,
  ...
  have hmeas : AEStronglyMeasurable (fun a : CoeffSpace d =>
    ...
    refine Measurable.aestronglyMeasurable ?_
    rw [hfeq]
    refine d124_measurable_recentre_sq F (respEhatPlus P jStar F t) (respxPlus P jStar F t e)
      (fun a => coarseBlockMatrix (respCell jStar F t) (respCoeffPlus F a)) ?_
    intro ζ δ
    have hx := h68_measurable_coarseBlockMatrix_plus hq t 0 F ζ δ
    rwa [b130_adaptedCellAtCenter_zero] at hx
  have hK0 : (0 : ℝ) ≤ respK0SqPlus P jStar F t := b179_blockSpecBound_nonneg _
  have hL0 : (0 : ℝ) ≤ respLsqPlus P jStar F t e :=
    d124_qform_nonneg hEhat.posSemidef (respxPlus P jStar F t e)
  have hbound : ∀ᵐ a ∂P, blockVecDot (respRecentrePlus P jStar F t e a (u a))
      (respRecentrePlus P jStar F t e a (u a))
      ≤ (respK0SqPlus P jStar F t * respLsqPlus P jStar F t e) *
          respAllScaleAbs P γ jStar F t a ^ 2 := by
    filter_upwards [d124_normalizedDefect_le_respAllScaleAbs_plus hd γ hγ P E Ψ Kg Src
      raw.stat raw.ell jStar raw.hj F hm t] with a ha
    have hpt := d124_recentre_sq_le_plus P jStar F t e hq hm hEhat a (u a) (hu a)
    have hn0 : (0 : ℝ) ≤ blockOpNorm (normalizedBlock
        (blockSub (coarseBlockMatrix (respCell jStar F t) (respCoeffPlus F a))
...
(`HCPoly/Entry/Multiscale/ResponseInputs/AdaptedSwarm.lean`, sorry-free, in the import closure of
this file through `AdaptedEnergy`):
  `1 ≤ κ_t ∧ κ_t ≤ κ_s ∧ κ_s ≤ r² κ_t ∧ 1 ≤ r ∧ r < exp(d σ)`.
CAVEAT: `response_imbalance_comparison` carries two premises that this statement -- and the statement in `AdaptedWeak.lean` --
...
`j_* < s`, so `Annealed.adaptedMean_antitone` (`HCPoly/Entry/Annealed/MeanOrder.lean`, which needs
`(jStar : ℤ) ≤ j ≤ k`) does not give `E_t ≤ E_s`, which is what `canonicalImbalance_mono`
(`AdaptedSwarm.lean`) consumes for `κ_t ≤ κ_s`.  `1 ≤ κ_t` alone does NOT need them.
...
conjuncts of `response_imbalance_comparison` (`AdaptedSwarm.lean`); it records, in compiled
...
theorem respK0Sq_mul_Lsq_le_arith {Cc Ce k0sq lsq kt ks : ℝ} (hCc : 0 < Cc) (_hCe : 0 < Ce)
  ...
  have hr1 : (1 : ℝ) ≤ Real.sqrt ks := by nlinarith [hrsq, hrnn, hks1]
  have hst : Real.sqrt kt ≤ Real.sqrt ks := Real.sqrt_le_sqrt hkts
  have hk0' : k0sq ≤ Cc * Real.sqrt ks := le_trans hk0 (by nlinarith [hst, hCc.le])
  ...
  have key : ∀ r : ℝ, 0 ≤ r → 1 ≤ r →
    ...
    intro r hr0 hr1'
    nlinarith [mul_nonneg (mul_nonneg hCc.le hr0) (sub_nonneg.mpr hr1')]
...
* the two scale-selection premises `hε`, `hσ` that this statement and the statement in `AdaptedWeak.lean` do not
  carry, and
* the single remaining routine matrix step `K_0^± ≤ C_c √κ_t` (hypotheses `hKm`, `hKp`), which is
  ...
  `h5rc_normalizedBlock_le_scale` (`AdaptedSwarm.lean`) followed by
  `blockSpecBound_le_of_loewner` (`AdaptedSwarm.lean`) -- both currently `private`, hence
...
`adaptedMean_persistence` Support:134 + `adaptedMean_antitone`, `t = s + H`).  If
...
`h5rc_normalizedBlock_le_scale` followed by `blockSpecBound_le_of_loewner`, and the constant is
`C = 2 C_c (1 + C_e)`, exactly as (b) below predicts.  A residual gap remains: the statement
(`AdaptedWeak.lean`) and the route assembly `response_weak_estimate_of_route` still do not
...
(a) `1 ≤ κ_t` and `κ_t ≤ κ_s` ARE derivable, with `C = 1`, from `RawOutput` alone -- they are
  conjuncts 1 and 2 of `response_imbalance_comparison` (`AdaptedSwarm.lean`,
  sorry-free) -- BUT only once the two scale-selection premises `hε : ε ∈ Set.Ioc 0 S.eps0` and
    ...
    (`AdaptedWeak.lean`) do not carry them.  See the note above this section and the
...
theorem respK0Sq_mul_Lsq_le_kappa (d : ℕ) (hd : 2 ≤ d) (γ : ℝ) (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
  ...
  refine ⟨2 * Cc + 2 * Cc * Ce, by nlinarith, ?_⟩
  ...
  have hm : (explicitCanonicalMetric F).PosDef := Geometry.explicitCanonicalMetric_posDef raw.symm raw.pos
  have hM0 : (toFullBlockMat (respM0 F)).PosDef := h4_respM0_full_posDef hm
  ...
  have hKm : respK0SqMinus P jStar F t ≤ Cc * Real.sqrt (respKappa P jStar F t) :=
    blockSpecBound_le_of_loewner _ _ hcnn
      (h5rc_normalizedBlock_le_scale _ _ hM0 _ hcalm)
  have hKp : respK0SqPlus P jStar F t ≤ Cc * Real.sqrt (respKappa P jStar F t) :=
    blockSpecBound_le_of_loewner _ _ hcnn
      (h5rc_normalizedBlock_le_scale _ _ hM0 _ hcalp)
  obtain ⟨hKm0, hKp0⟩ := respK0Sq_nonneg P jStar F t
