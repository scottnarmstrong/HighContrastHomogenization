import HCPoly.Entry.Multiscale.ResponseInputs.HC3_RowsOscCoord
import HCPoly.Entry.Multiscale.ResponseInputs.HC3_RowsMeanRowCar
import HCPoly.Entry.Multiscale.ResponseInputs.HC3_RowsOptMean
import HCPoly.Entry.Multiscale.ResponseInputs.HC3_DirectCellHalfClean
import HCPoly.Entry.Multiscale.ResponseInputs.HC3_DirectOscCrossCar
import HCPoly.Entry.Multiscale.ResponseInputs.HC3_DirectCutoffWeights
import HCPoly.Entry.Multiscale.ResponseInputs.HC3_DirectLoadNonneg
...
`HC3_DirectCellHalfClean` bounds the cell half by `(L_s^∓ 2 tau^∓)^{1/2}` from the carriers' own
annealed data.  The oscillation half of `HC3_RowsOscHalfCar`, read at the dual variable `Y^∓`,
bounds the scalar annealed functional `⨍_{U_t}(φ-1)(⟨Y₂, ∇u⟩ + ⟨Y₁, a∇u⟩)` against its depth-`H`
cell part; `HC3_DirectOscCrossCar` identifies the crossed pairing of the vector oscillation part
...
`HC3_RowsOscCoord`, the row assembly of `HC3_RowsMeanRowCar` then gives the named obligation
  ...
  · -- the oscillation half
    ...
    have hcrossOn : ∀ a : CoeffSpace d, IntegrableOn
      ...
      have h := integrableOn_cross_optimizerField_respCoeffMinus_adaptedCellAtCenter jStar hjStar F hm
        t 0 a (uM a) (respYMinus P jStar F t e) (b130_zero_mem_triadicIndexBox 0)
      ...
      have hsplit := volumeAverage_fluct_eq_osc_add_cell (respGrid jStar F) hq t H φ
        ...
        ((hcrossOn a).bdd_mul (c := 1)
          ...
          (Filter.Eventually.of_forall fun x => by
            ...
            exact ⟨by linarith [hφ.1 x], by linarith [hφ.2.1 x]⟩))
        (fun w hw => by
          ...
          have hA := ((hcrossOn a).mono_set hsub).bdd_mul (c := 2)
            ...
            (Filter.Eventually.of_forall fun x => by
              rw [Real.norm_eq_abs, abs_le]
              exact ⟨by linarith [hφ.1 x], hφ.2.1 x⟩)
  ...
  · -- the oscillation half
    ...
    have hcrossOn : ∀ a : CoeffSpace d, IntegrableOn
      ...
      have h := integrableOn_cross_optimizerField_respCoeffPlus_adaptedCellAtCenter jStar hjStar F hm
        t 0 a (uP a) (respYPlus P jStar F t e) (b130_zero_mem_triadicIndexBox 0)
      have hUcell : adaptedCellAtCenter (respGrid jStar F) (t - ((0 : ℕ) : ℤ)) 0
          = respCell jStar F t := by
        rw [Nat.cast_zero, sub_zero]
        exact b130_adaptedCellAtCenter_zero (respGrid jStar F) t
