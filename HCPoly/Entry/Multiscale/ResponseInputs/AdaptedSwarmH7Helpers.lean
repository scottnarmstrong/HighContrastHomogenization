...
private theorem h7_blockCongr_blockD (A : BlockMat d) :
    blockCongr (blockD d) A = blockMatFlipFlux A := by
  have hD : toFullBlockMat (blockD d) = Matrix.fromBlocks (1 : Mat d) 0 0 (-1) := by
    rw [h7_toFullBlockMat_eq_fromBlocks]
    rfl
  rw [blockCongr, hD, h7_toFullBlockMat_eq_fromBlocks A, Matrix.fromBlocks_transpose,
  ...
  rw [h7_ofFullBlockMat_fromBlocks]
  rfl

/-- `D · G_{-g} = G_g · D` in the full `2d × 2d` carrier: both are `[[Id, 0], [g, -Id]]`. -/
private theorem h7_blockD_mul_shear_neg (g : Mat d) :
  ...
  have hG : ∀ c : Mat d, toFullBlockMat (⟨1, 0, c, 1⟩ : BlockMat d)
      = Matrix.fromBlocks (1 : Mat d) 0 c 1 := fun c => h7_toFullBlockMat_eq_fromBlocks _
...
theorem h7_annealedBlockOf_respCoeffMinus_adapted [NeZero d]
  ...
  refine h7_annealedBlockOf_eq_blockCongr _ _ hint fun a => ?_
  exact h7_coarseBlockMatrix_sub_skew_eq_blockCongr hgskew
    (h7_hasQuadraticMu_adaptedCellTranslate q hq j y a)
...
theorem h7_annealedBlockOf_respCoeffPlus_adapted [NeZero d]
    {P : Measure (CoeffSpace d)} (q : Mat d) (hq : IsUnit q) (j : ℤ) (y : Vec d)
    (F : BlockMat d) (hF : (toFullBlockMat F).PosDef)
    (hint : HasIntegrableCoarseBlock P (HighContrast.adaptedCellTranslate q j y)) :
  ...
  have hb : ∀ a : CoeffSpace d,
    ...
    intro a
    have hquad := h7_hasQuadraticMu_adaptedCellTranslate q hq j y a
    ...
    have h2 := h7_coarseBlockMatrix_sub_skew_eq_blockCongr (U := V)
    ...
    have h3 : coarseBlockMatrix V (adjointCoeffField (⇑a.1 : CoeffField d))
      ...
      rw [coarseBlockMatrix_adjointCoeffField_of_exists
        (exists_coarseBlockMatrix_of_hasQuadraticMu hquad), ← h7_blockCongr_blockD]
      rfl
    rw [h1, h2, h3, h7_blockCongr_blockCongr]
  rw [h7_annealedBlockOf_eq_blockCongr _ (respCoeffPlus F) hint hb, blockAdjoint,
    h7_blockCongr_blockCongr]
  exact congrArg (fun G => blockCongr G (annealedBlock P V))
    (congrArg ofFullBlockMat (h7_blockD_mul_shear_neg (respg F)))


/-! ### Round 1c: the `k ≥ j_*` branch — the determinant-drift Loewner comparison. -/

/-- Trace form of `h5_mean_part_le_determinantDrift` (`work.lean`): the same telescoping,
stopping one step earlier, so that the conclusion is a *Loewner* envelope rather than the
infimum `blockSpecBound`.  (`p.response.transfer`.) -/
private theorem h7_mean_trace_le_determinantDrift (d : ℕ) (hd : 2 ≤ d) (γ : ℝ)
    (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
    ...
    (3 : ℝ) ^ (-(respRho γ * ((t : ℝ) - (k : ℝ)))) *
        blockTrace (blockSub (normalizedMean P (Geometry.explicitRoundedGrid jStar mt) k t)
  ...
  have hgap : ∀ i j : ℤ, (jStar : ℤ) ≤ i → i ≤ j →
      (toFullBlockMat (blockSub (normalizedMean P q i t)
        (normalizedMean P q j t))).PosSemidef := by
    intro i j hi hij
    simpa only [normalizedMean] using
      h5_normalizedBlock_gap_posSemidef (adaptedMean P q i) (adaptedMean P q j)
        ...
        (Annealed.adaptedMean_antitone d hd P γ E Ψ K Src hstat hdag jStar hjStar mt hm i j
          hi hij)
  set T : ℤ → ℝ := fun j =>
    blockTrace (blockSub (normalizedMean P q (j - 1) t) (normalizedMean P q j t)) with hTdef
  have hT0 : ∀ j : ℤ, (jStar : ℤ) + 1 ≤ j → 0 ≤ T j := by
    intro j hj
    exact Analysis.blockTrace_nonneg (hgap (j - 1) j (by omega) (by omega))
  have hI : normalizedMean P q t t = Book.Ch02.blockIdentity d :=
    Annealed.normalizedMean_self d hd P γ E Ψ K Src hstat hdag jStar hjStar mt hm t
  have hstep1 : blockTrace (blockSub (normalizedMean P q k t) (Book.Ch02.blockIdentity d))
      = ∑ j ∈ Finset.Icc (k + 1) t, T j := by
    have hsum : ∑ j ∈ Finset.Icc (k + 1) t, T j =
        ∑ j ∈ Finset.Icc (k + 1) t,
          (blockTrace (normalizedMean P q (j - 1) t) - blockTrace (normalizedMean P q j t)) := by
      refine Finset.sum_congr rfl fun j _ => ?_
      simp only [hTdef, h5_blockTrace_blockSub]
    rw [hsum, h5_telescope_Icc (fun j => blockTrace (normalizedMean P q j t)) k t hkt,
      ← hI, h5_blockTrace_blockSub]
  have hrho : (1 : ℝ) / 2 ≤ respRho γ := by
    unfold respRho; linarith [hγ.1]
  have hw : ∀ j ∈ Finset.Icc (k + 1) t,
    ...
    have hu : (0 : ℝ) ≤ (t : ℝ) - (j : ℝ) := by linarith
    have hv : (0 : ℝ) ≤ (t : ℝ) - (k : ℝ) := by linarith
    have s1 : ((1 - γ) / 8) * ((t : ℝ) - (j : ℝ)) ≤ (1 / 8 : ℝ) * ((t : ℝ) - (j : ℝ)) := by
      nlinarith [hγ.1]
    have s2 : (1 / 8 : ℝ) * ((t : ℝ) - (j : ℝ)) ≤ (1 / 8 : ℝ) * ((t : ℝ) - (k : ℝ)) := by
      linarith
    have s3 : (1 / 8 : ℝ) * ((t : ℝ) - (k : ℝ)) ≤ respRho γ * ((t : ℝ) - (k : ℝ)) := by
      nlinarith
    linarith
  have hc0 : (0 : ℝ) ≤ (3 : ℝ) ^ (-(respRho γ * ((t : ℝ) - (k : ℝ)))) :=
    Real.rpow_nonneg (by norm_num) _
  calc
    (3 : ℝ) ^ (-(respRho γ * ((t : ℝ) - (k : ℝ)))) *
        blockTrace (blockSub (normalizedMean P q k t) (Book.Ch02.blockIdentity d))
        = (3 : ℝ) ^ (-(respRho γ * ((t : ℝ) - (k : ℝ)))) * ∑ j ∈ Finset.Icc (k + 1) t, T j := by
      rw [hstep1]
    _ = ∑ j ∈ Finset.Icc (k + 1) t,
          (3 : ℝ) ^ (-(respRho γ * ((t : ℝ) - (k : ℝ)))) * T j := Finset.mul_sum _ _ _
    _ ≤ ∑ j ∈ Finset.Icc (k + 1) t,
...
`(R^{-1/2})⁻¹`, exactly as in the closing step of `le_detRatio_smul_of_le` (`work.lean`). -/
private theorem h7_le_smul_of_normalizedBlock_le {A R : BlockMat d} {c : ℝ}
  ...
  have hcongr := congr_le h (matSqrt Rf⁻¹)⁻¹
  rw [hSit] at hcongr
...
private theorem h7_blockIdentity_posDef :
  ...
  have hq : blockVecDot X (blockMatVecMul (Book.Ch02.blockIdentity d) X)
    ...
    rw [← dotProduct_toFullBlockVec, toFullBlockVec_blockMatVecMul,
      h7_toFullBlockMat_blockIdentity, Matrix.one_mulVec]
  rw [hq]
...
theorem h7_adaptedMean_le_of_drift (d : ℕ) (hd : 2 ≤ d) (γ : ℝ)
    (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
    (P : Measure (CoeffSpace d)) [IsProbabilityMeasure P]
    (E : BlockMat d) (Ψ : ℝ → ℝ) (K : ℝ) (Src : CoeffSpace d → ℝ)
    (hstat : IsStationaryLaw P) (hdag : CoarseEllipticityDagger P γ E Ψ K Src)
    ...
    BlockMatLoewnerLE (adaptedMean P (Geometry.explicitRoundedGrid jStar mt) k)
      (blockScale (1 + (3 : ℝ) ^ (respRho γ * ((t : ℝ) - (k : ℝ))) *
  ...
  have hI : normalizedMean P q t t = Book.Ch02.blockIdentity d :=
    Annealed.normalizedMean_self d hd P γ E Ψ K Src hstat hdag jStar hjStar mt hm t
  set G : BlockMat d := blockSub (normalizedMean P q k t) (Book.Ch02.blockIdentity d) with hG
  have hgapPSD : (toFullBlockMat G).PosSemidef := by
    rw [hG, ← hI]
    simpa only [normalizedMean] using
      h5_normalizedBlock_gap_posSemidef (adaptedMean P q k) (adaptedMean P q t)
  ...
  have hblk : BlockMatLoewnerLE G (blockScale Tr (Book.Ch02.blockIdentity d)) := fun X =>
    (h5_loewner_le_blockOpNorm G hgapPSD X).trans
      (Source.blockScale_le_blockScale_of_pos h7_blockIdentity_posDef hopLe X)
  have hfull : toFullBlockMat G ≤ Tr • (1 : Matrix (BlockCoord d) (BlockCoord d) ℝ) := by
    have h := (Annealed.fullBlock_le_iff hgapPSD.isHermitian
      (by rw [h5_toFullBlockMat_blockScale_identity]
          exact h7_hermitian_smul_one _)).2 hblk
    rwa [h5_toFullBlockMat_blockScale_identity] at h
  have hnorm : toFullBlockMat (normalizedMean P q k t)
      ≤ (1 + Tr) • (1 : Matrix (BlockCoord d) (BlockCoord d) ℝ) := by
    have hsub : toFullBlockMat G
        = toFullBlockMat (normalizedMean P q k t)
          - (1 : Matrix (BlockCoord d) (BlockCoord d) ℝ) := by
      rw [hG, h5_toFullBlockMat_blockSub, h7_toFullBlockMat_blockIdentity]
  ...
  have hres : toFullBlockMat (adaptedMean P q k)
    ...
    h7_le_smul_of_normalizedBlock_le (hpd t) hnorm
  have hstep : BlockMatLoewnerLE (adaptedMean P q k)
    ...
    · rw [h7_toFullBlockMat_blockScale]; exact h7_hermitian_smul (hpd t).isHermitian _
    · rwa [h7_toFullBlockMat_blockScale]
  have hdrift : Tr ≤ (3 : ℝ) ^ (respRho γ * ((t : ℝ) - (k : ℝ))) *
      determinantDrift P γ q jStar t := by
    have h := h7_mean_trace_le_determinantDrift d hd γ hγ P E Ψ K Src hstat hdag jStar hjStar
      mt hm k t hk hkt
    have hpow : (0 : ℝ) < (3 : ℝ) ^ (respRho γ * ((t : ℝ) - (k : ℝ))) :=
      Real.rpow_pos_of_pos (by norm_num) _
    have hinv : (3 : ℝ) ^ (-(respRho γ * ((t : ℝ) - (k : ℝ))))
        = ((3 : ℝ) ^ (respRho γ * ((t : ℝ) - (k : ℝ))))⁻¹ := by
      rw [Real.rpow_neg (by norm_num)]
    rw [hinv] at h
    rw [← hTrdef] at h
    calc Tr = (3 : ℝ) ^ (respRho γ * ((t : ℝ) - (k : ℝ))) *
          (((3 : ℝ) ^ (respRho γ * ((t : ℝ) - (k : ℝ))))⁻¹ * Tr) := by
          field_simp
      _ ≤ (3 : ℝ) ^ (respRho γ * ((t : ℝ) - (k : ℝ))) * determinantDrift P γ q jStar t :=
...
theorem h7_tsum_two_regime {u : ℕ → ℝ} {N : ℕ} {a b r₁ r₂ : ℝ}
...
theorem h7_blockScale_mono {A B : BlockMat d} (h : BlockMatLoewnerLE A B) {c : ℝ}
...
theorem h7_blockScale_blockScale (c c' : ℝ) (A : BlockMat d) :
    blockScale c (blockScale c' A) = blockScale (c * c') A := by
  simp only [blockScale, smul_smul]

private theorem h7_adaptedCellAtCenter_zero (q : Mat d) (j : ℤ) :
...
private theorem h7_zero_mem_triadicIndexBox (n : ℕ) :
...
theorem h7_adaptedCell_subset (q : Mat d) {j s : ℤ} (h : j ≤ s) :
    HighContrast.adaptedCell q j ⊆ HighContrast.adaptedCell q s := by
  have hn : s - ((s - j).toNat : ℤ) = j := by omega
  have := h5_adaptedCellAtCenter_subset_adaptedCell q s (s - j).toNat
    (h7_zero_mem_triadicIndexBox (d := d) (s - j).toNat)
  rwa [hn, h7_adaptedCellAtCenter_zero] at this
...
theorem h7_annealedBlock_le_adaptedMean (d : ℕ) (hd : 2 ≤ d) (γ : ℝ)
    (hγ : γ ∈ Set.Ico (0 : ℝ) 1) :
    ∃ Csrc C : ℝ, 0 < Csrc ∧ 0 < C ∧
      ∀ (P : Measure (CoeffSpace d)) [IsProbabilityMeasure P]
  ...
  have hwinJ : HighContrast.adaptedCell (Geometry.explicitRoundedGrid jStar m) (jStar : ℤ) ⊆
      HighContrast.centeredCube d (2 * (jStar : ℤ)) :=
    subset_trans (h7_adaptedCell_subset (Geometry.explicitRoundedGrid jStar m) hjs) hwin
  ...
  have hstep : BlockMatLoewnerLE
    ...
    have := h7_blockScale_mono h2
      (c := C₁ * (3 : ℝ) ^ (γ * max ((jStar : ℝ) - (k : ℝ)) 0))
      (by positivity)
    rw [h7_blockScale_blockScale] at this
    exact this X
  ...
  have hecc2 : Real.sqrt (‖m‖ * ‖m⁻¹‖) ≤ ‖m‖ * ‖m⁻¹‖ := by
    nlinarith [Real.sq_sqrt (by positivity : (0:ℝ) ≤ ‖m‖ * ‖m⁻¹‖)]
...
theorem h7_respSourceLoad_le_of_loewner_scalewise (P : Measure (CoeffSpace d)) (jStar : ℕ)
  ...
  have hbound : ∀ n : ℕ, ∀ z ∈ triadicIndexBox d n,
    ...
    intro n z hz
    have h1 := h7_qform_fst (hres n z hz) Y.1
    have h2 := h7_qform_snd (hres n z hz) Y.2
    rw [h7_blockScale_qform_upperLeft] at h1
    rw [h7_blockScale_qform_lowerRight] at h2
    constructor
    · refine h1.trans ?_
    ...
    · refine h2.trans ?_
      have hle : vecDot Y.2 (matVecMul (respM0 F).lowerRight Y.2) ≤ M := by
        ...
        linarith
      exact mul_le_mul_of_nonneg_left hle (hc n)
  exact h7_respSourceLoad_le_scalewise P jStar F s b Y (fun n => c n * M)
...
theorem h7_profile_nonneg (d : ℕ) (hd : 2 ≤ d) (γ : ℝ) (hγ : γ ∈ Set.Ico (0 : ℝ) 1)
    (P : Measure (CoeffSpace d)) [IsProbabilityMeasure P]
    (E : BlockMat d) (Ψ : ℝ → ℝ) (K : ℝ) (Src : CoeffSpace d → ℝ)
...
theorem h7_blockCongr_mono {A B : BlockMat d} (G : BlockMat d)
    (h : BlockMatLoewnerLE A B) : BlockMatLoewnerLE (blockCongr G A) (blockCongr G B) := by
  intro X
  rw [h7_blockVecDot_blockCongr, h7_blockVecDot_blockCongr]
  exact h _

private theorem h7_ofFullBlockMat_smul (c : ℝ)
    (M : Matrix (BlockCoord d) (BlockCoord d) ℝ) :
    ofFullBlockMat (c • M) = blockScale c (ofFullBlockMat M) := by
  refine blockMat_ext ?_ ?_ ?_ ?_ <;> rfl

theorem h7_blockCongr_blockScale (G : BlockMat d) (c : ℝ) (A : BlockMat d) :
    blockCongr G (blockScale c A) = blockScale c (blockCongr G A) := by
  rw [blockCongr, blockCongr, h7_toFullBlockMat_blockScale, ← h7_ofFullBlockMat_smul]
  congr 1
  rw [Matrix.mul_smul, Matrix.smul_mul]

theorem h7_respM0_isSymm {F : BlockMat d} (hm : (explicitCanonicalMetric F).PosDef) :
...
theorem h7_respM0_blockPosDef {F : BlockMat d} (hm : (explicitCanonicalMetric F).PosDef) :
