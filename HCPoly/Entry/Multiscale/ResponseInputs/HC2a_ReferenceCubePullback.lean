...
Paper `p.response.transfer`. Depends on `HC2_WeakSeminorm.lean` (B4).
-/
...
theorem opNorm_metric_product_le {l : ℕ} (hl : 2 * d ≤ 3 ^ l) {m : Mat d} (hm : m.PosDef) :
  ...
  have h1 : ‖L⁻¹ * q‖ ≤ 3 / 2 := by
    ...
    calc ‖(1 : Mat d) + B‖ ≤ ‖(1 : Mat d)‖ + ‖B‖ := norm_add_le _ _
      _ ≤ 3 / 2 := by rw [norm_one_mat]; linarith
  have hBlt : ‖-B‖ < 1 := by rw [norm_neg]; linarith
...
repo-standing `2 * d ≤ 3 ^ l` (`HCPoly/Entry/Geometry/RoundedGrid.lean`) is available, which gives
...
omit [NeZero d] in
theorem matVecMul_inv_apply {q : Mat d} (hq : IsUnit q) (x : Vec d) :
...
omit [NeZero d] in
theorem matVecMul_apply_inv {q : Mat d} (hq : IsUnit q) (x : Vec d) :
    matVecMul q (matVecMul q⁻¹ x) = x := by
  have hdet : IsUnit q.det := (Matrix.isUnit_iff_isUnit_det q).mp hq
  rw [Geometry.matVecMul_eq_mulVec, Geometry.matVecMul_eq_mulVec, Matrix.mulVec_mulVec,
...
def matMeasurableEquiv {q : Mat d} (hq : IsUnit q) : Vec d ≃ᵐ Vec d where
  toFun := matVecMul q
  invFun := matVecMul q⁻¹
  left_inv := matVecMul_inv_apply hq
  right_inv := matVecMul_apply_inv hq
...
theorem injective_matVecMul {q : Mat d} (hq : IsUnit q) :
  ...
  rwa [matVecMul_inv_apply hq, matVecMul_inv_apply hq] at h
...
theorem isUnit_det_matSqrt {m : Mat d} (hm : m.PosDef) : IsUnit (matSqrt m).det := by
  have hScfc : matSqrt m = CFC.sqrt m := matSqrt_eq_cfc_sqrt hm.posSemidef
  have hSposdef : (matSqrt m).PosDef := by rw [hScfc]; exact Geometry.posDef_sqrt hm
  exact (Matrix.isUnit_iff_isUnit_det _).mp hSposdef.isUnit
...
omit [NeZero d] in
theorem blockVecDot_self_nonneg (Y : BlockVec d) : 0 ≤ blockVecDot Y Y := by
...
theorem integrableOn_apply_of_vecNormSq_le {U : Set (Vec d)} (hUfin : volume U ≠ ⊤)
  ...
  have h3 : |Y x i| ≤ (1 + (Y x i) ^ 2) / 2 := by
    nlinarith [sq_nonneg (|Y x i| - 1), sq_abs (Y x i)]
  rw [Real.norm_eq_abs]
  linarith
