...
`Audit/AlgebraicConvergence/` carries a Mathlib-only comparator challenge for
...
lake env comparator Audit/AlgebraicConvergence/comparator.json
```
...
artifacts of `Audit/AlgebraicConvergence` before the run, so that the challenge
