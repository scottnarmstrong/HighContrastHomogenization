...
import Audit.Support.AlgebraicConvergenceBridge
import HCPoly.MainResults

/-!
# Algebraic convergence at a polynomial scale: the proof of the Mathlib-only statement

This file proves the statement of `Audit.AlgebraicConvergence.Challenge`, in
the vocabulary rebuilt over Mathlib alone in
`Audit.AlgebraicConvergence.SolutionBasic`, from the library theorem
`HCPoly.algebraic_convergence` through the identifications of
`Audit.Support.AlgebraicConvergenceBridge`.
